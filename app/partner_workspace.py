...
from camera_mapping import CameraNumberConflict, assign_camera_number
...
def register_partner_workspace_routes(app: FastAPI, shell: Callable) -> None:
    ...
    def customer_account(request: Request):
        identity=partner_identity(request)
        if not identity or identity.get('role') not in {'customer_owner','customer_viewer'}: return RedirectResponse('/partner-login',status_code=303)
        ...
        activated=row("SELECT id FROM appliances WHERE customer_id=? AND activation_status='activated'",(customer['id'],))
        if identity['role']=='customer_owner' and not activated: return RedirectResponse('/customer/setup',status_code=303)
    ...
    def customer_first_setup(request: Request):
        ...
        camera_rows=''.join(f'<tr><td>{escape(c["name"])}</td><td><input class="setup-camera-name" data-id="{c["id"]}" value="{escape(c["name"],quote=True)}"></td><td><select class="setup-camera-site">'+''.join(f'<option value="{s["id"]}" {"selected" if s["id"]==c["site_id"] else ""}>{escape(s["name"])}</option>' for s in sites)+'</select></td><td>'+escape(c.get('status') or 'pending')+f'</td><td><input class="setup-camera-number" data-id="{c["id"]}" type="number" min="1" max="256" value="{c.get("camera_number") or ""}" placeholder="Not assigned"></td></tr>' for c in cameras) or '<tr><td colspan="5">No cameras discovered or preconfigured yet.</td></tr>'
        ...
        <div class="customer-setup-step" data-step="5" hidden><h2>Camera setup</h2><div style="overflow:auto"><table class="data-table"><thead><tr><th>Camera</th><th>Rename</th><th>Location/site</th><th>Status</th><th>Local camera slot #</th></tr></thead><tbody>{camera_rows}</tbody></table></div><p class="health-detail">Recording, retention, analytics, and notifications use the selected customer plan. Per-camera overrides can be added after activation.</p><button class="ghost-button" id="save-camera-setup">Save camera setup</button></div>
    ...
    def configure_customer_cameras(request: Request,payload: dict) -> dict:
        ...
        camera_items=payload.get('cameras',[]); now=datetime.now().isoformat(); errors=[]
        with connection() as db:
            for item in camera_items:
                camera_id=str(item.get('id','')); existing=db.execute('SELECT id,appliance_id FROM cameras WHERE id=? AND customer_id=?',(camera_id,identity['customer_id'])).fetchone()
                if not existing: continue
                db.execute('UPDATE cameras SET name=?,site_id=?,resolution=?,status=? WHERE id=?',(item.get('name','Camera'),item.get('site_id'),item.get('resolution','2mp'),item.get('status','configured'),camera_id))
                if 'camera_number' in item:
                    try: assign_camera_number(db,camera_id,item.get('camera_number'),appliance_id=existing['appliance_id'],customer_id=identity['customer_id'])
                    except (ValueError,LookupError,CameraNumberConflict) as error: errors.append({'camera_id':camera_id,'error':str(error)})
        audit(identity,'camera.configuration_saved','customer',identity['customer_id'],{'count':len(camera_items),'errors':len(errors)})
        return {'message':'Camera setup saved.' if not errors else 'Camera setup saved with camera slot errors.','errors':errors}
