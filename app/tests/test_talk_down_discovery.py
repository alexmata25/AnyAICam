...
# --------------------------------------------- real, DB-provisioned camera credential resolution


def test_provisioned_camera_credentials_resolves_from_the_real_encrypted_credential_table(tmp_path, monkeypatch):
    """2026-09-16: this module ported onto the current architecture --
    the original (env-var-only) _camera_credentials() predates the
    dynamic camera-provisioning/encrypted-credential work. Proves the
    real, current path: a camera provisioned through the same
    cameras/camera_credentials tables main.py's own
    _provisioned_camera_stream() reads for the real RTSP stream."""
    ...
    db_path = tmp_path / "test_talk_down_discovery_provisioned.db"
    with override_target(sqlite_path=str(db_path)):
        ...
        with connection() as db:
            ...
            db.execute(
                ...
                "VALUES('cam-1','c1','s1','a1',1,'Camera 1','rtsp://192.168.0.38:554/ch1','2026-01-01')"
            )
            db.execute(
        ...
        result = discovery._camera_credentials(1)
    assert result == ("192.168.0.38", "admin", "hunter2")


def test_camera_credentials_falls_back_to_legacy_env_vars_when_not_provisioned(tmp_path, monkeypatch):
    from database_backend import override_target
    from partner_db import initialize_database

    db_path = tmp_path / "test_talk_down_discovery_fallback.db"
