...
def _seed_plan(conn, customer_id="cust-1", camera_quantity=5):
    """Legacy, partner-quoted plan row -- pre-dates the Local/Hybrid
    camera-slot architecture and is never touched by a real Stripe
    purchase (see customer_entitlements.py's own module docstring, audit
    finding #2). expected_camera_count no longer reads this column at
    all (see test_expected_camera_count_ignores_the_legacy_plans_table_
    below) -- kept here only for tests that seed it deliberately
    mismatched from the real entitlement, to prove that."""
    conn.execute(
...
def _seed_entitlement(conn, customer_id="cust-1", camera_slot_quantity=5, product="camera_slots_local"):
    """The real, Stripe-verified source of camera-slot capacity --
    total_camera_slots() sums every *active* row here, never
    plans.camera_quantity."""
    conn.execute(
        "INSERT INTO customer_entitlements(id,customer_id,product,camera_slot_quantity,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
        (f"ent-{customer_id}-{product}", customer_id, product, camera_slot_quantity, "active", "2026-01-01", "2026-01-01"),
    )
    conn.commit()
...
def test_renamed_placeholder_saved_through_step5_never_counts_as_configured(http_client, db_path):
    ...
    _seed_tenant(conn)
    _seed_entitlement(conn, camera_slot_quantity=5)
...
def test_a_real_camera_not_yet_marked_configured_status_still_counts(http_client, db_path):
    ...
    conn = sqlite3.connect(db_path)
    _seed_tenant(conn)
    _seed_entitlement(conn, camera_slot_quantity=1)
...
# --------------------------------------- expected_camera_count's real source


def test_expected_camera_count_ignores_the_legacy_plans_table(http_client, db_path):
    """Regression for a second confirmed-live bug found in the same
    audit: expected_camera_count used to read plans.camera_quantity --
    a legacy, partner-quoted estimate that predates Local/Hybrid and is
    never touched by a real Stripe purchase. A customer who bought
    "Local 1-8" (8 real, Stripe-verified slots) but whose onboarding
    record happened to carry a stale/placeholder plans.camera_quantity
    of 1 saw "0 of 1 cameras configured" instead of "0 of 8". Deliberately
    seeds the two values in conflict (plans=1, entitlement=8) to prove
    expected_camera_count follows the real entitlement, never plans."""
    conn = sqlite3.connect(db_path)
    _seed_tenant(conn)
    _seed_plan(conn, camera_quantity=1)
    _seed_entitlement(conn, camera_slot_quantity=8, product="camera_slots_local")
    response = http_client.get("/api/customer/cameras", cookies={partner_portal.SESSION_COOKIE: _owner_cookie()})
    data = response.json()
    assert data["expected_camera_count"] == 8


def test_expected_camera_count_sums_only_active_entitlements(http_client, db_path):
    # Two DIFFERENT products (customer_entitlements is idempotent per
    # (customer_id,product) -- a real customer_id+product pair can never
    # carry two rows, so "cancelled and inactive" is modeled here as a
    # separate service line, e.g. a cancelled Hybrid entitlement
    # alongside an active Local one, exactly like a customer who
    # switched plans would look.
    conn = sqlite3.connect(db_path)
    _seed_tenant(conn)
    _seed_entitlement(conn, camera_slot_quantity=32, product="camera_slots_hybrid")
    conn.execute("UPDATE customer_entitlements SET status='cancelled' WHERE product='camera_slots_hybrid'")
    _seed_entitlement(conn, camera_slot_quantity=8, product="camera_slots_local")
    conn.commit()
    response = http_client.get("/api/customer/cameras", cookies={partner_portal.SESSION_COOKIE: _owner_cookie()})
    data = response.json()
    assert data["expected_camera_count"] == 8  # the cancelled 32-slot Hybrid row must never be counted
