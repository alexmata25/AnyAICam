...
def test_start_message_creates_a_session_with_no_video_flags_in_ffmpeg_command(monkeypatch):
    ...
    monkeypatch.setattr(client, "_camera_transport_credentials", lambda n: ("10.0.0.1", 554, "/", "user", "pass"))
...
def test_stop_message_ends_only_that_session(monkeypatch):
    ...
    def fake_build_transport(host, port, rtsp_path, username, password):
        t = _FakeTransport()
...
# 2026-09-16: ported onto the current architecture -- metadata never
# actually carried rtsp_port/rtsp_path in practice (talk_down_discovery.py's
# real probe only ever populates profile_token/audio_output_token/
# send_primacy/output_token/name), so _build_transport()'s own
# metadata-based override was dead code that always fell back to the
# default. The real port/path now come from the camera's own already-
# configured onvif_endpoint RTSP URL (the exact same one the live video
# stream uses), via _camera_transport_credentials() --
# _provisioned_camera_transport_credentials() specifically. These tests
# now cover that real resolution instead of the metadata path it
# replaced.

def test_build_transport_uses_exactly_the_host_port_path_it_is_given():
    built = client._build_transport("10.0.0.1", 554, "/onvif/audio", "user", "pass")
    assert built.host == "10.0.0.1"
    assert built.port == 554
    assert built.rtsp_path == "/onvif/audio"
    assert built.username == "user"
    assert built.password == "pass"


def test_provisioned_transport_credentials_parses_host_port_path_from_the_real_onvif_endpoint(tmp_path, monkeypatch):
    ...
    db_path = tmp_path / "test_talk_audio_relay_client_provisioned.db"
    with override_target(sqlite_path=str(db_path)):
        initialize_database()
        with connection() as db:
            db.execute("INSERT OR IGNORE INTO partners(id,name,created_at) VALUES('p1','P','2026-01-01')")
            db.execute("INSERT OR IGNORE INTO customers(id,partner_id,name,email,status,created_at) VALUES('c1','p1','C','c@example.test','active','2026-01-01')")
            db.execute("INSERT OR IGNORE INTO sites(id,customer_id,name,created_at) VALUES('s1','c1','S','2026-01-01')")
            db.execute("INSERT OR IGNORE INTO appliances(id,customer_id,site_id,cloud_id,created_at) VALUES('a1','c1','s1','AIC-1','2026-01-01')")
            db.execute(
                "INSERT INTO cameras(id,customer_id,site_id,appliance_id,camera_number,name,onvif_endpoint,created_at) "
                "VALUES('cam-1','c1','s1','a1',1,'Camera 1','rtsp://192.168.0.38:8554/ch1','2026-01-01')"
            )
            db.execute(
                "INSERT INTO camera_credentials(camera_id,encrypted_blob,created_at,updated_at) VALUES('cam-1',?,'2026-01-01','2026-01-01')",
                (encrypt_camera_credentials("admin", "hunter2"),),
            )
        result = client._camera_transport_credentials(1)
    assert result == ("192.168.0.38", 8554, "/ch1", "admin", "hunter2")


def test_transport_credentials_falls_back_to_legacy_env_vars_when_not_provisioned(tmp_path, monkeypatch):
    from database_backend import override_target
    from partner_db import initialize_database

    db_path = tmp_path / "test_talk_audio_relay_client_fallback.db"
    monkeypatch.setenv("CAMERA3_HOST", "10.0.0.3")
    monkeypatch.setenv("CAMERA3_USERNAME", "admin")
    monkeypatch.setenv("CAMERA3_PASSWORD", "legacy-pass")
    monkeypatch.setenv("CAMERA3_PATH", "/Streaming/Channels/101")
    with override_target(sqlite_path=str(db_path)):
        initialize_database()
        result = client._camera_transport_credentials(3)
    assert result == ("10.0.0.3", 554, "/Streaming/Channels/101", "admin", "legacy-pass")
