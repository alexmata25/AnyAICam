...
def _provisioned_camera_transport_credentials(camera_number: int) -> tuple[str, int, str, str, str] | None:
    """Real, provisioned-camera source: looks up the cameras row
    assigned this camera_number plus its encrypted credentials (the
    same two-table lookup main.py's own _provisioned_camera_stream()
    uses for the real RTSP video stream -- duplicated here rather than
    imported, matching this project's established convention for
    edge-worker modules that must stay import-safe/circular-import-
    free without pulling in the whole monolith). Host/port/path are
    all parsed from the camera's own real onvif_endpoint RTSP URL --
    the actual, currently-configured stream endpoint, not a guessed or
    hardcoded default -- so the backchannel targets the exact same
    host/port this camera's live video RTSP session already uses.
    None if this camera_number has no provisioned camera, no stored
    credential, or an onvif_endpoint that isn't a real rtsp:// URL."""
    ...
    try:
        with connection() as db:
            camera = db.execute(
                "SELECT id, onvif_endpoint FROM cameras WHERE camera_number=?",
                (camera_number,),
            ).fetchone()
            if not camera:
                return None
    ...
    if not credentials:
        return None
    parsed = urlsplit(camera["onvif_endpoint"] or "")
    if parsed.scheme != "rtsp" or not parsed.hostname:
        return None
    return (
        parsed.hostname, parsed.port or RTSP_PORT_DEFAULT, parsed.path or "/",
        credentials.get("username", ""), credentials.get("password", ""),
    )


def _camera_transport_credentials(camera_number: int) -> tuple[str, int, str, str, str] | None:
    """Resolves (host, port, rtsp_path, username, password) for a
    configured camera slot: the real, dynamically-provisioned camera
    registry first (see _provisioned_camera_transport_credentials()),
    falling back to legacy CAMERA{N}_HOST/USERNAME/PASSWORD/PATH env
    vars (port fixed at RTSP_PORT_DEFAULT, matching camera_url()'s own
    legacy-path convention) for backward compatibility with an
    installation that configured cameras that way -- the exact same
    dual-path precedence camera_url() itself already establishes."""
    provisioned = _provisioned_camera_transport_credentials(camera_number)
    if provisioned:
        return provisioned
    try:
    ...
    if not host or not username:
        return None
    path = os.environ.get(f"CAMERA{camera_number}_PATH", "/")
    return host, RTSP_PORT_DEFAULT, path, username, password
...
def _build_transport(host: str, port: int, rtsp_path: str, username: str, password: str) -> OnvifBackchannelTransport:
    """The one, and currently only, concrete transport this milestone
    builds -- a future vendor-specific fallback would be a second
    branch here, selected by what the discovered talk_down_metadata
    says (e.g. an absent ONVIF audio_output_token), never by which
    camera number this happens to be."""
    return OnvifBackchannelTransport(host=host, port=port, username=username, password=password, rtsp_path=rtsp_path)
...
def _start_session(session_id: str, camera_id: str, metadata: dict, sample_rate: int, camera_map: dict[int, dict]) -> bool:
    ...
    transport = _build_transport(host, port, rtsp_path, username, password)
