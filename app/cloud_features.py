...
<script>const csrf=()=>document.cookie.split('; ').find(x=>x.startsWith('anyaicam_csrf='))?.split('=')[1]||'';document.getElementById('forgot-form').addEventListener('submit',async e=>{{e.preventDefault();const response=await fetch('/api/password-reset/request',{{method:'POST',headers:{{'Content-Type':'application/json','X-CSRF-Token':csrf()}},body:JSON.stringify({{email:document.getElementById('forgot-email').value}})}}),r=await response.json(),box=document.getElementById('message');box.textContent=r.message||'If the account exists, a reset message has been prepared.';box.style.display='block'}});</script>
</body></html>''')
