...
unit-tested directly without Docker, unlike main.py (which hardcodes
/app/... paths at import time and requires the project's Docker image).

Ported 2026-09-21 from the divergent, unmerged `analytics-rules-
foundation-20260821` branch onto this branch unmodified (this file and
its own test_analytics_rules_engine.py) -- it was written and proven
there against the legacy, non-tenant-safe `AnalyticsRuleModel`/
analytics_rules.json shape (`analytic_type`, bare integer `camera`,
`confidence_threshold`, no customer_id), which this module still
speaks natively since it is deliberately camera_number/rule-dict
agnostic about where a rule dict actually came from. The real caller
on THIS branch is `customer_analytics_rules_worker.py`, which adapts
each tenant-scoped `customer_analytics_rules` row (rule_type, no
confidence_threshold) into this exact same rule-dict shape before
calling `evaluate_rules()` -- this module itself was not changed to
do that translation, keeping it equally reusable by either rule store.
`update_tracker()` attaches a stable per-camera track_id to each
cycle's raw pixel-coordinate detections, and `evaluate_rules()` is
called every cycle with the tracked detections and this camera's
currently-configured rules to get back zero or more rule-triggered
events, which the caller then persists through the same
`append_analytics_event()` storage path every other event type in
