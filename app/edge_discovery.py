"""Bounded ONVIF/WS-Discovery and RTSP fingerprinting helpers for edge camera discovery.

Stage 1 scope (see docs/CLAUDE_HANDOFF.md and the Stage 1 PR summary):

- Everything in this module is a pure function/class or takes injectable probe
  callables. Nothing here performs a network call unless the caller supplies
  (or explicitly invokes) one of the real probe implementations
  (``ws_discover``, ``open_ports``, ``onvif_metadata``).
- ``main.py``'s live ``/api/camera-discovery/scan`` route does NOT call
  ``ws_discover``, ``onvif_metadata``, or ``CameraDiscoveryService`` — it only
  uses the network-free helpers (``candidate_stream_urls``, the URL
  sanitizers). The WS-Discovery/ONVIF probe path and ``CameraDiscoveryService``
  exist here so they are available, testable, and injectable for a later,
  separately-approved stage; they are intentionally not wired into any live
  route yet.
- No credential handling lives in this module. There is no
  ``credentialed_rtsp_url`` and no persistent credential store here.
"""
...
def candidate_stream_urls(host: str, manufacturer: str, port: int = 554) -> list[str]:
    """Common per-vendor RTSP path guesses, computed with zero network calls."""
    paths = {
...
# ---------------------------------------------------------------------------
# Real network probes. Ported for architectural completeness and future
# stages. NOT called from app/main.py's live discovery route in Stage 1.
# ---------------------------------------------------------------------------


def ws_discover(timeout: float) -> list[dict]:
    """Send a WS-Discovery multicast probe and collect ONVIF responses.

    Not invoked by the live v1.1.0 scan route in Stage 1 — see module docstring.
    """
...
def open_ports(host: str, ports: tuple[int, ...], timeout: float) -> list[int]:
    """Bounded TCP connect probe. Not invoked directly by main.py, which has
    its own equivalent (`v110_probe_host`); kept here for `CameraDiscoveryService`.
    """
    found = []
    for port in ports:
        try:
...
def onvif_metadata(xaddr: str, timeout: float) -> dict:
    """Fetch manufacturer/model/stream URLs from a real ONVIF device.
...
class CameraDiscoveryService:
    """Injectable orchestration of WS-Discovery + port scan + ONVIF metadata.

    Defaults to the real network probes above, but every probe is a
    constructor argument specifically so tests (and any future caller) can
    supply fakes and never touch a real socket. Not instantiated by
    app/main.py's live route in Stage 1 — see module docstring.
    """
