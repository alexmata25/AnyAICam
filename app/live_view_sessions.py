...
def _customer_identity(request: Request) -> dict:
    """Accepts customer_owner AND customer_viewer -- per-camera authorization
    (ownership + the can_live permission row) is enforced immediately after
    by _authorized_camera() below, which already fails closed for
    customer_viewer with no explicit grant. This function only rejects
    requests that aren't a portal customer identity of either role at all."""
...
def register_live_view_session_routes(app: FastAPI) -> None:
    ...
    def stop_live_view(request: Request, session_id: str) -> dict:
        ...
        with connection() as db:
            ...
            # Mark this viewer stopped before deciding whether the underlying
            ...
            # ends this viewer session. Relay shutdown is owned by
            # live_relay_idle_sweep.py, which applies the idle grace period
            # and re-checks that no requested viewers remain before stopping.
