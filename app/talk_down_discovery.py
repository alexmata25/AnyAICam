...
def _provisioned_camera_credentials(camera_number: int) -> tuple[str, str, str] | None:
    """Real, provisioned-camera source: looks up the cameras row
    assigned this camera_number plus its encrypted credentials (the
    exact same two-table lookup main.py's own
    _provisioned_camera_stream() uses for the real RTSP stream --
    duplicated here rather than imported, matching this file's own
    established convention of duplicating control-plane/credential
    helpers instead of importing main, which recording_uploader.py/
    analytics_sync.py already do for the same reason: this module must
    stay import-safe and circular-import-free without pulling in the
    entire monolith). Returns (host, username, password) with only the
    hostname taken from onvif_endpoint -- ONVIF SOAP calls use their
    own port (ONVIF_PORT, default 80), never the RTSP port embedded in
    that URL. None if this camera_number has no provisioned camera or
    ...
    if not credentials:
        return None
    host = urlsplit(camera["onvif_endpoint"] or "").hostname
    if not host:
        return None
    return host, credentials.get("username", ""), credentials.get("password", "")


def _camera_credentials(camera_number: int) -> tuple[str, str, str] | None:
    """Resolves (host, username, password) for a configured camera
    slot: the real, dynamically-provisioned camera registry first (see
    _provisioned_camera_credentials()), falling back to legacy
    CAMERA{N}_HOST/USERNAME/PASSWORD env vars for backward
    compatibility with an installation that configured cameras that
    way -- the exact same dual-path precedence camera_url() itself
    already establishes for RTSP. No camera number is ever special-
    cased beyond selecting which row/env-var prefix to read; a camera
    with neither source configured is read as "not configured", never
    guessed at."""
    provisioned = _provisioned_camera_credentials(camera_number)
    if provisioned:
        return provisioned
    try:
