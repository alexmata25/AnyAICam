...
def register_live_view_page_routes(app: FastAPI, page_shell: Callable) -> None:
    ...
    def live_view_page(request: Request, camera_id: str):
        ...
        # Matches Phase 6b/6c's own exact role requirement -- this
        # page's JavaScript calls those SAME routes, so gating any
        # broader here (e.g. also allowing customer_viewer, as
        # /customer-account's own redirect check does) would only
        # render a page whose every start/stop call then 403s.
        ...
        content = (
            ...
            f'<a class="ghost-button" href="/customer-account">Back</a></header>'
...
(function(){{
  ...
  async function stopSession(isUnload){{
    ...
    if(isUnload){{
      ...
      else{{try{{fetch(url,{{method:'POST',keepalive:true}})}}catch(e){{}}}}
    }}else{{
