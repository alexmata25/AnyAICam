...
Installed release history since: `aa4dc2edb135e379631a9d17db8923e56a202ed9` (fixed cloud-provisioned cameras reaching the edge VMS's own local DB — "Option B" local credential handoff), then **`6d6dcd5d6e665bfb74ab76cb6dd1fb2bc3c01d6c`** (current, verified live 2026-09-13 — fixes the `aa4dc2e` handoff's own two blocking bugs: the local endpoint's `PUBLIC_PATH_PREFIXES` gap and Docker hairpin-NAT gateway detection; also fixes camera-provisioning to consume a licensed placeholder instead of always inserting a new row). Repair install run by the operator (`sudo ./install.sh --repair`); full post-install verification (build_id, `appliance_identity.json` SHA-256 byte-for-byte unchanged, agent re-auth/heartbeat resumed, no crash loop, local camera/credential counts unchanged, new endpoint's auth confirmed both present and still gated by the real bearer credential) recorded in `docs/PROJECT_CHECKPOINT.md`'s own 2026-09-13 dated entries — read those for full detail, this file is deliberately just the pointer.

**Current real state, for the next session**: one real camera (`AIC-C814766E`'s "Camera 1", device_key `urn:uuid:b3464000-5074-11b4-82cc-142ffda2f6af`) is provisioned cloud-side but has **no local credential yet** (`camera_not_bound`) — its one-time local handoff failed under the old, buggy code before this fix existed, and that handoff is never automatically retried. Recovery (re-click "Add this camera" in Customer Setup Step 4 for the same already-discovered device) and a related one-row placeholder-count reconciliation are both designed but **explicitly not yet authorized/executed** — see `docs/PROJECT_CHECKPOINT.md` for the exact proposed procedure before doing either.
