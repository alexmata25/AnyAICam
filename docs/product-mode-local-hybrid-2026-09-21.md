...
**Verified live on Ryzen, 2026-09-21 (read-only; nothing deleted or changed)** -- the real active/licensed camera count is **5**, cross-checked across three independent sources, with the filesystem used only as supporting evidence exactly as instructed:

| Source | Finding |
|---|---|
| Local `cameras` table (`/app/recordings/partner_portal.db`, this appliance's own mirror of the cloud's `cameras` rows via `edge_camera_sync.py`) | 9 total identity rows for this `customer_id`/`appliance_id`. **5** have `camera_number` (1-5), a real `device_key`, `status='configured'`, and a real ONVIF/RTSP endpoint to a real device IP (Living Room, Driveway Right, Driveway Left, Bedroom, Front Door). The other 4 (`camera_number IS NULL`, `device_key IS NULL`, `status='pending_installation'`, named "Camera 3"/6/7/8) are purchased-but-never-installed placeholder slots -- the exact same placeholder shape `_customer_playback_cameras()`'s own docstring already documents excluding via `camera_number IS NOT NULL`. |
| DB active-camera filter (`camera_number IS NOT NULL`) | **5** -- exact match. |
| Currently enabled camera mapping on Ryzen (`ANYAICAM_RECORDING_UPLOAD_CAMERAS` env var) | `1,2,3,4,5` -- exact match. |
| Filesystem `camera*` directories under `/app/recordings` (supporting evidence only, per instruction) | 12 directories exist, but only `camera1`-`camera5` have ever contained a file (fresh writes within the last 10 minutes at check time); `camera6`-`camera12` each have **zero files, ever** (not stale-from-past-use, just empty scaffolding that was never populated -- they don't even correspond numerically to the 4 pending-installation placeholder rows, which never received a `camera_number` at all). |

All three non-filesystem sources agree exactly: **5 active cameras**, not 12. The 7 extra directories are dead disk artifacts with no corresponding live identity anywhere in the configuration -- left in place, not deleted, per instruction.

Look up this Ryzen unit's real `customer_id` (via `appliances.customer_id` in the cloud DB, keyed by its `cloud_id`) to actually perform the grant below -- that lookup itself requires cloud DB access this session didn't have (the EC2/cloud host was unreachable via Tailscale at verification time).
...
- **Direct entitlement grant (what's actually usable right now)**: an admin/support action calling `customer_entitlements.upsert_entitlement(customer_id=<real id>, product="camera_slots_local", camera_slot_quantity=8, status="active")` directly against the cloud database -- `camera_slot_quantity=8` (the "1-8" tier's `camera_slot_maximum`), not `5`: the stored quantity represents licensed tier capacity, matched against `PLAN_TIERS` by `camera_slot_maximum` elsewhere in this codebase (see `partner_workspace.py`'s own `tier_label` lookup), not a live count of currently-installed cameras -- 5 active cameras fit comfortably within the 8-slot tier, with headroom for the 4 already-purchased-but-uninstalled placeholders. This is the same function the real checkout webhook would have called -- there is no separate/lesser code path, so a manually-granted entitlement is indistinguishable from a Stripe-driven one to every downstream reader (`product_mode_for_customer()`, the dashboard's Plan stat, `/subscription-portal`).
