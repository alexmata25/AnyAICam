...
- **Ryzen config** (`/etc/anyaicam/vms.env`): `ANYAICAM_RECORDING_UPLOAD_CAMERAS` still reads `1` only -- **blocked**, not attempted around. This file is root-owned; the project's own established non-privileged, no-sudo SSH key gets `Permission denied` on a plain `ls`, and `sudo` itself reports "a password is required." This is a real, live-confirmed privilege boundary, the same one `AUTONOMOUS_VALIDATION_PERMISSIONS.md` already describes for Ryzen, not a session/credential expiry like the AWS side was. **Until this file is edited by someone with root/sudo on the appliance (or that access is explicitly granted), Cameras 2-5 will not actually attempt bulk/continuous recording upload** even though both cloud-side layers (IAM + pilot-camera list) now allow it -- `recording_upload_worker()`'s own per-camera scope check (`RECORDING_UPLOAD_CAMERA_SCOPE`) skips any camera number not in that env var before it ever reaches the credentials endpoint. Exact one-line change needed: `ANYAICAM_RECORDING_UPLOAD_CAMERAS=1,2,3,4,5`, then restart the `anyaicam-vms` container (`docker compose restart vms` from `/opt/anyaicam`, or an equivalent recreate).
...
Three genuine blockers remain, none designed around, all documented precisely enough to act on immediately once resolved:
1. **Ryzen root/sudo access** -- needed for both the bulk-recording camera-scope env var and (optionally) raising event-media retry throughput. One line each, exact values above.
2. **A decision on RDM live-toggle testing** -- requires fabricating a subscription/entitlement record on the real customer's account since none exists today; not done without that decision.
3. **Physical-world camera stimulus** -- the consolidated list above.

Everything not blocked on one of those three was continued and verified: bulk-recording IAM+cloud-config (2 of 3 layers), event-media (fully working, draining, just not yet caught up to real-time), five-camera parity (Live View/Playback/Dashboard, 15/15 passing), the full e2e suite (75 passed, 3 legitimate skips, 0 failed, re-run twice this pass with no regressions), and stability evidence (clean across every dimension checked). Production was never touched. No camera credential, firmware, or destructive action was taken at any point.
