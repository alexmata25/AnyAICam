...
First attempt at this session's own quota work added `DISTINCT` directly inside `get_camera_numbers()` -- caught by that exact guard test before committing, and reverted. RDM camera-quota enforcement will use the same established `sorted(set(...))` idiom at its own, new call site instead of touching the shared primitive, matching precedent rather than re-litigating it.

## Milestone: RDM Camera Quota built, verified end-to-end, and deployed to staging; a real hard-delete bug found and fixed in camera removal (2026-09-16)

### The quota-enforcement machinery already existed -- audited first, built only the missing link

Per the standing rule, searched all branches before writing anything. Found this was mostly already built and already on the authoritative branch: `customer_entitlements.total_camera_slots()` (sums active `customer_entitlements.camera_slot_quantity` rows) is already the single source of truth two independent, fail-closed, real gates already enforce (`request_camera_provisioning()` in partner_workspace.py, request-time; `appliance_submit_provisioning()` in appliance_cloud.py, confirmation-time re-check closing the race the first gate alone can't) -- Phase 7 work from earlier in this project, 5 existing tests, still green. Camera discovery is also already correctly edge/Ryzen-driven (`GET`/`POST /api/appliance/{cloud_id}/scan-jobs`; the setup wizard's own copy: "The browser does not scan the local network. Camera discovery runs on the assigned appliance.") -- no changes needed there either.

**What was actually missing**: any way for an RDM administrator to set that enforced number directly. `customer_entitlements.py`'s own module docstring already documented why: it is deliberately, additively separate from `plans.camera_quantity` (a pre-existing partner-facing "Update camera entitlement" form, `PUT /api/partner/customers/{id}/plan`) -- and `total_camera_slots()` has never read `plans` at all. An admin using that existing form changes only what the customer portal *displays*; the real enforcement gates never see it. Confirmed this is a real, live gap, not a hypothetical: real Ryzen customer `d75bdbecdd4887de4d2b89a9fcea9092` has a real `camera_slots_local: 8` entitlement (why their 5 real cameras were never blocked) that a partner/RDM admin has had no direct way to change without faking a Stripe event.

**Built**: `POST /api/admin/customers/{customer_id}/camera-quota` (administrator-only), reusing `upsert_entitlement()` unchanged under a dedicated `camera_slots_rdm` product key -- becomes enforced immediately, zero changes to either existing gate. Proven with real HTTP requests through both the new admin route and the real customer-facing provisioning routes, matching the user's exact specified sequence: 5/5 reject camera 6; remove one -> 4/5 -> permit one replacement; RDM raises quota 5->8 -> permit cameras 6-8; 8/8 reject camera 9. **Quota-decrease safety, resolved rather than escalated**: proven that lowering the quota below the current active count never deletes/disables existing cameras -- the enforcement gates only ever block *new* additions, nothing in the codebase ever deletes a camera in response to a quota change, so this was already safe by construction; not a product-policy gap needing a new decision.

### Found and fixed a real, previously-untested hard-delete bug during the removal audit

While auditing "customer removes a camera, frees the slot" for the same quota work, found `DELETE /api/customer/cameras/{camera_id}` ran `DELETE FROM cameras WHERE id=?` -- even though `recordings`, `detection_events`, and `customer_clip_jobs` all carry a real `FOREIGN KEY(camera_id) REFERENCES cameras(id)` (db_migrations.py). This is exactly the "camera removal must not destroy historical recordings/events/audit history" requirement, previously unenforced and untested (no test file for this route existed at all). Fixed to a soft removal: `device_key`/`camera_number` cleared (frees the slot -- `device_key IS NOT NULL` is exactly what both quota gates and `configured_camera_count` count) and `status` becomes `'removed'`, but the camera row itself -- and everything that references its id -- is kept permanently. A `'removed'` status also naturally prevents a future discovery pass from silently recycling that exact row for a *different* physical camera (the existing placeholder-slot-reuse query in `appliance_submit_provisioning()` is scoped to `status='pending_installation'`, so it already correctly skips a removed row). Also added the missing `status!='removed'` filter to the two customer-facing surfaces that would otherwise have shown a removed camera as a live ghost entry (`list_customer_cameras()`, the notification-settings camera picker).

20 new tests (11 quota route + end-to-end lifecycle, 9 removal), full existing camera-provisioning/slot-enforcement suites still green. The pre-existing 73-failure baseline (confirmed via `git stash` A/B in the prior commit) is unchanged.

**Deployed to staging** (`b89fa4d`), zero-downtime cutover (`portal-b89fa4d` now holds the `portal`/`vms` aliases; `portal-82c5873` renamed+stopped as rollback). **Not yet deployed to Ryzen** -- these routes are cloud-portal-facing (RDM admin action, customer provisioning/removal), not part of Ryzen's own edge streaming/recording path, so this is a lower-urgency sync than the recording-mode/Talkdown fixes were; Ryzen remains correctly on `82c5873` and fully healthy. Did not mutate the real Ryzen customer's live entitlement data to "test" this in production -- verification is the 20 new tests plus the clean staging build/deploy/health-check, which is sufficient without touching real customer state.

### State to resume from

Talk-down, RDM Recording-Mode, and RDM Camera Quota (admin control + safe removal) are all source-complete, tested, and deployed to staging; Ryzen is current through the recording-mode/Talk-down milestone (`82c5873`) with the camera-quota commit (`b89fa4d`) staged but not yet installed there. Per the user's own priority order, remaining: customer event email/SMS notifications, then password recovery + admin customer-account controls. FR/AACO remain paused, fully preserved both locally and on GitHub.

## Milestone: Live-view architecture fully traced with real evidence; P2P live-view signaling foundation built, tested, and deployed to staging (2026-09-16/17)

### Live-video architecture audit (explicit user request, no changes made during the audit itself)

User reported already being able to remotely view Ryzen's cameras and asked for the REAL current transport to be traced (not assumed), specifically whether Cloudflare carries the live video and whether AWS participates. Traced end-to-end with live evidence rather than code-only inference:

- Recovered `docs/AI_HANDOFF.md` §8 from branch `build/v1.2-modular-foundation` (missing on this branch since an earlier cleanup) -- the original, already-approved architecture decision record for exactly this system, comparing 7 transport options (WebRTC/SFU, AWS KVS+WebRTC, SRT, RTMP, HLS-relay, custom tunnel, AWS IVS) and choosing appliance-direct-to-S3 HLS-segment-relay + CloudFront signed delivery, with WebRTC explicitly noted as "a possible future low-latency tier... added later behind the same authorization model," never built at the time.
- Confirmed that design is now fully live and real, not just documented: `ANYAICAM_LIVE_RELAY_ENABLED=true` + `ANYAICAM_CLOUDFRONT_URL`/`ANYAICAM_CLOUDFRONT_KEY_PAIR_ID`/signing-key role+secret all genuinely configured on staging; issued a real, live STS credential from the control-plane session endpoint; found real uploaded segment objects (~4-4.5MB/segment, real recent camera motion) in the actual bucket (`anyaicam2026`, NOT the recordings bucket -- a separate bucket this session hadn't audited before); confirmed a real 1-day S3 lifecycle rule already expires the `live/` prefix (no runaway cost); found 2,636 real `live_view_sessions` rows and 2,911 real relay commands in the staging DB, spanning all 5 real cameras.
- Answer to the specific questions: Cloudflare only fronts `portal-staging.anyaicam.com`'s small control-plane/page HTTP traffic (confirmed via DNS -- Cloudflare anycast IPs); it never carries a video byte. Actual video segments go browser-direct to a separate, dedicated Amazon CloudFront distribution, never through Cloudflare or FastAPI. AWS fully participates (S3 + CloudFront + STS + Secrets Manager), and cost follows viewership (relay only runs while ≥1 viewer is watching), not camera count or 24/7 recording -- but per-segment size (~16 Mbps) is real, ongoing AWS spend for every minute a customer watches, which is the concrete economic case for P2P.
- One process note self-flagged to the user: a diagnostic script printed a real (but short-lived, ~20-min TTL, single-camera-prefix-scoped) STS credential into the session transcript while tracing this live. It expired naturally; flagged for transparency rather than treated as nothing.

### P2P live-view foundation -- built, tested, committed (`1c9d956`), deployed to staging

Per explicit user direction: direct P2P (STUN-only, TURN deliberately deferred until real NAT-failure/relay-usage data justifies it) as the PREFERRED transport for both Local and Hybrid, racing in parallel against the now-proven relay path above (never sequentially, never delaying/displacing a relay that already won), full instrumentation (which transport a session used, connect time, failure), and the whole layer architected so TURN is a config addition later, not a redesign.

Searched all branches/history first, per standing rule -- confirmed zero existing WebRTC/STUN/TURN/signaling work anywhere to reuse; this is genuinely new.

Built: `db_migrations.py`'s `20260917_live_view_p2p` (new `p2p_attempted`/`relay_bytes` columns; `live_view_sessions`' existing `transport`/`ready_at`/`failed_at`/`error` columns -- previously-unused placeholders from the original relay design -- are now actually written to; new `live_view_p2p_signaling` append-only offer/answer/ICE table). `app/live_view_p2p.py`: customer-facing offer/ICE/answer-poll/transport-outcome routes and appliance-facing poll-pending/answer/ICE routes, both reusing existing auth (`partner_identity` cookie / `authenticate_appliance` bearer+nonce) rather than a new auth boundary; `ice_servers()` STUN-only by default, TURN addable via env vars with zero other code changes. `live_view_page.py` (both the multi-camera grid and single-camera pages): session start now races a direct P2P attempt against the existing relay poll concurrently; first to actually produce video wins via a `claimTransport()` choke point, the loser tears itself down, outcome is reported for the instrumentation.

Deliberately NOT built: an appliance-side WebRTC publisher (nothing yet produces a real SDP answer with media -- would need e.g. MediaMTX or GStreamer's webrtcbin reading the same local camera pipeline `start_live_stream()` already maintains, plus installer changes to run it on the appliance). Until that exists, every real P2P attempt times out client-side and falls back to relay automatically -- the correct, safe interim state, not a bug. **Nothing has been installed on the real Ryzen appliance for this feature** -- that step needs an explicit go-ahead before it touches production appliance software, per the user's own stated bar for autonomous action.

16 new tests; full-suite A/B via `git stash` confirms zero new genuine regressions -- the only diff between with/without this change was `test_claim_flow_end_to_end.py`'s pre-existing test-order flakiness (confirmed passes 100% standalone, unrelated to this change). Feature-flagged off by default (`ANYAICAM_LIVE_P2P_ENABLED=false`) -- zero behavior change in production until explicitly enabled. Deployed to staging (`portal-1c9d956` now holds the `portal`/`vms` network aliases; `portal-4a84a81-pre-p2p-foundation` kept as rollback, never deleted); verified live externally (`/version` and the new `/api/customer/live/p2p/config` route both respond correctly through `https://portal-staging.anyaicam.com`).

### State to resume from (superseded by the milestone below)

Next real step for P2P: decide and build the appliance-side WebRTC publisher (MediaMTX vs. GStreamer webrtcbin), wire it to the new appliance-facing signaling routes, test off of Ryzen's production path first, then get explicit approval before installing on the real Ryzen appliance. Customer Event Notifications remains a separate, still-open milestone (engine/logic source-complete and tested from the prior session; user is mid-flow on Step 1 of 3 for their own portal/env configuration; `notification_retry_worker` still needs to be re-wired into `main.py`'s task lifecycle before that milestone's own commit).

## Milestone: appliance-side WebRTC publisher (MediaMTX bridge) built and tested; staging login outage caused by my own deploy mistake, root-caused and fixed (2026-09-17)

### Appliance-side WebRTC publisher (`6c34131`)

Per explicit product direction: MediaMTX (not a custom media server) bridges the cloud P2P signaling exchange to real RTSP/WebRTC media. `app/webrtc_publisher.py` -- MediaMTX config generation (API/WebRTC HTTP loopback-only, STUN-only ICE by default, TURN addable via env vars with zero code change), subprocess lifecycle (spawn/monitor/restart), per-camera path sync driven entirely off the appliance's own known-camera set (camera_id-keyed, no hardcoded IDs, works for any fleet size), WHEP offer/answer bridge, best-effort ICE forwarding. Camera RTSP credentials flow through exactly once (an injected `camera_url()` callable, same circular-import-avoidance pattern already used elsewhere in main.py's route registration) straight into MediaMTX's local config API -- never logged, returned, or exposed to the browser. `live_view_page.py`'s `attemptLiveP2P()` now waits for local ICE gathering to complete before sending the offer (non-trickle by design), sidestepping a byte-exact RFC 8840 trickle implementation for v1.

Protocol details (WHEP route shapes: `POST/PATCH/DELETE /{path}/whep[/{secret}]`; config REST API: `POST/DELETE /v3/config/paths/(add|delete)/{name}`; `webrtcICEServers2` field names) were verified against MediaMTX's real current source (`bluenviron/mediamtx` v1.21.0), not guessed from documentation summaries. **What is NOT verified**: a live run against the real `mediamtx` binary -- this sandbox could not reliably download the release asset (repeated truncated transfers across 3 attempts, exit code 56 each time) despite otherwise-working internet access. Tests instead run against a real local HTTP server (stdlib `http.server`, a background thread) that reproduces MediaMTX's confirmed real route contract byte-for-byte, plus a fake `subprocess.Popen` for process-lifecycle tests -- 25 new tests, including the safety-critical proof that the worker spawns nothing and polls nothing while `ANYAICAM_LIVE_P2P_ENABLED` is unset. Full-suite regression: 91 failed/2226 passed, exactly matching the established baseline -- zero new failures.

P2P remains feature-flagged off and **nothing has been installed on the real Ryzen appliance**. The next required step before any Ryzen involvement is running the real Linux `mediamtx` binary somewhere (Ryzen itself in an isolated, not-wired-to-the-VMS-service location, or another Linux box) and validating this module's WHEP/config-API assumptions against real HTTP responses, not just the confirmed-from-source contract.

### Staging login outage -- caused by my own deployment mistake, not a code regression

While the publisher work above was in progress, the user reported customer login broken on staging. Root-caused within minutes: the P2P-foundation deploy (`portal-1c9d956`, from `1c9d956`) mounted the database volume at the wrong container-internal paths (`/app/recordings/db` and `/app/data-config`) instead of the real ones (`/app/data` and `/opt/anyaicam/data/config`, confirmed against the last known-good container's actual mounts via `docker inspect`) -- a mistake in my own deploy command, not a defect in the committed application code. The new container silently started against an empty, unpersisted SQLite database, so every login and every real Ryzen appliance call (heartbeat, config sync, live relay session request) failed closed with 401/403 -- correct fail-closed behavior given the actual (wrong) database state, which is why nothing crashed or 500'd, just uniformly rejected everything.

Fixed by redeploying the same already-built image (`deploy-portal:1c9d956`, no rebuild needed) with the correct mounts; verified before cutover that the real data was present (5 `partner_users`, 3 `appliances`, 4 `customers`, 23 `cameras`, including the real customer's own row), and after cutover that the real Ryzen appliance's `GET /api/appliance/configuration` calls flipped from `403 Forbidden` back to `200 OK` within seconds. User confirmed login works. Broken container preserved (never deleted) as `portal-1c9d956-broken-wrong-mounts`; live container renamed cleanly back to `portal-1c9d956`.

### State to resume from (superseded by the milestone below)

Give the user the P2P findings + phased deployment plan (binary verification first, off Ryzen's live path; then installer wiring; then a single-camera controlled Ryzen test with relay fallback confirmed throughout; then broaden to all 5 cameras) -- explicitly not proceeding to any of those phases without the user's own go-ahead per their standing instruction. Customer Event Notifications remains the separate, still-open milestone described above, unchanged by this session's P2P work.

## Milestone: MediaMTX real-binary verification (Phase 1) -- passed, one real bug found and fixed (2026-09-17)

Per explicit instruction: validated `webrtc_publisher.py` (commit `6c34131`) against the actual current Linux `mediamtx` v1.21.0 binary, not the fake test server -- isolated on Ryzen (own directory `~/mediamtx-p2p-test`, own ports 8889/9997/8189, never connected to the live `anyaicam-vms` container or wired into systemd, `ANYAICAM_LIVE_P2P_ENABLED` never touched), fully cleaned up afterward.

**Every required behavior verified real and correct:**
- Config API add/delete: real `POST/DELETE /v3/config/paths/(add|delete)/{name}` returns exactly the status codes this module already expected (200 "ok"; idempotent 404 "path not found" on a second delete).
- A config file generated by this module's own `render_mediamtx_config()` (not a hand-written test config) is accepted by the real binary, which starts and listens on exactly the configured loopback ports.
- WHEP offer/answer against a REAL Ryzen camera (camera 1, then camera 2 concurrently) via its real RTSP credentials: 201 Created, a relative `Location` header (matches `_resolve_location()`'s handling), and a real SDP answer containing the camera's actual negotiated H264 (1280x720 Baseline) track -- confirmed via the path's own runtime stats (`ready:true`, real `inboundBytes` counted from the live camera, not a stub).
- ICE/STUN: the answer SDP already carried every candidate MediaMTX gathered before responding -- including a genuine STUN-discovered server-reflexive candidate off Ryzen's real public IP -- exactly matching this module's non-trickle-by-design assumption.
- Dynamic per-camera path creation/removal: proven independent for two real cameras simultaneously; each is on-demand (an idle path with no active reader stopped pulling RTSP on its own after MediaMTX's own idle-close timeout -- confirmed via inboundBytes dropping to 0).
- RTSP credential handling: the real camera URL (with real embedded credentials) was fetched once from the live VMS container, written straight to a locally-permissioned file, and used only as an HTTP request body read from that file -- never printed, echoed, or logged anywhere in this session's own output at any point; grepped the real WHEP response bodies afterward to confirm no RTSP URL leaked into them.
- Cleanup when a session ends: MediaMTX auto-reaps an abandoned/never-fully-connected WHEP session on its own (a DELETE against the already-reaped session correctly 404s) -- real confirmation of the design decision to rely on natural WebRTC disconnect detection rather than building explicit close signaling.
- Failure behavior: an unreachable camera's WHEP POST returns a real HTTP 400 after MediaMTX's own ~10s `sourceOnDemandStartTimeout` (not a hang); MediaMTX being killed entirely produces a real connection-refused, which `_config_request()`/`whep_offer()` already handle via their existing `URLError`/`HTTPError` branches.

**One real bug found and fixed**: MediaMTX's real ~10s failure latency for an unreachable camera meant `_bridge_tick()`'s previously-synchronous call chain (`_control_plane_get` -> `_handle_pending_signal` -> `whep_offer`) could block this worker's entire asyncio event loop -- shared by every other background task in the same VMS process -- for up to 10 seconds per stuck signal. Fixed: both calls now run via `asyncio.to_thread()`, the same pattern `_refresh_camera_map()`/`sync_camera_paths()` already used. New regression test (`test_bridge_tick_never_blocks_the_event_loop_on_a_slow_signal`) proves a concurrently-scheduled asyncio task still runs promptly even while a pending signal is genuinely blocked. 26 tests in `test_webrtc_publisher.py` (was 25), full relevant-suite regression clean.
...
Both P2P phases the user authorized (1: real-binary verification: passed; 2: installer wiring: built/tested) are complete. Remaining before any real Ryzen P2P test: build an actual release that includes a real MediaMTX binary, deploy it to Ryzen (repair-install, matching the established division of labor), confirm the binary lands correctly and MediaMTX still does not start, then get explicit approval for the single-camera controlled test with `ANYAICAM_LIVE_P2P_ENABLED` -- none of that has been requested yet. Customer Event Notifications remains the separate, still-open milestone described earlier in this document.

## Milestone: automatic local recording storage management -- built, tested, deployed to staging (2026-09-17)

Per explicit instruction (safe defaults, build/test without touching real Ryzen recordings, only enable deletion after verified): audited first (`recording_retention_sweep.py` is cloud/S3-only; the appliance-agent already heartbeats real `disk_capacity`/`disk_used`/`recording_used`; the cloud already shows a 90%-used "low disk" warning -- all reused, nothing rebuilt; no local automatic cleanup existed anywhere before this).

**Built** (`81c1f82`): `local_storage_policy.py` (RDM-configurable reserved/warning free-space percentages, 10%/20% system defaults, mirrors `event_media_policy.py`'s override pattern exactly) and `local_storage_manager.py` (the core worker) -- monitors the real recordings filesystem, and once free space hits the reserved floor, deletes the oldest eligible local recordings **across every camera in one true global chronological order** (never per-camera), so no single camera can hog the disk. Eligibility is deliberately restricted to Local/Hybrid-tier cameras (`cloud_recording_mode != 'continuous'`) -- Continuous/Cloud-tier files are never touched in v1 rather than risk deleting a paying customer's only copy without verifying upload completion; that tier gets a real `critical` alert instead. Every deletion removes the matching `recordings` catalog row (Playback never shows a broken entry) and is logged to a new `local_storage_cleanup_log` audit table. Two independent, fail-closed flags: `ANYAICAM_LOCAL_STORAGE_MANAGEMENT_ENABLED` gates monitoring entirely; `ANYAICAM_LOCAL_STORAGE_AUTO_DELETE_ENABLED` separately gates only deletion, so a real rollout can verify reported numbers with monitoring-only before ever enabling deletion.

**RDM/cloud side**: heartbeat now carries the appliance's own real-time `storage_state`/`storage_free_percent`/`storage_last_cleanup_at` (a cross-process handoff from the VMS app to the appliance-agent via a state file, the mirror of the existing `live_relay_commands.json` convention); `GET /api/appliance/configuration` gains a `storage_policy` field (same sync channel as `cloud_policy`); new `POST /api/admin/customers/{id}/storage-policy` RDM route; the appliance-dashboard health cards now show Healthy/Warning/Cleanup Active/Critical plus free%. Also fixed a real, previously-silent notification gap while wiring this: `notification_engine.py`'s `SUPPORTED` set was missing `'storage_problem'`, the exact label `notification_preferences.py`'s own customer-facing settings page has always used -- a customer who opted in to "Recording/storage problem" alerts could never have actually received one, the same class of dead-wiring gap `'ppe'` fixed earlier this session.

**Tested**: 31 new tests, entirely against simulated disk usage (monkeypatched `shutil.disk_usage`) and `pytest tmp_path` -- no real Ryzen recording, threshold, or disk state touched at any point. Full-suite regression: 91 failed/2266 passed, exact match against the established baseline list.

**Deployed to staging** (`portal-81c1f82`, verified via the new `deploy/verify_cutover_safety.py` mandatory pre-cutover check -- `CUTOVER_OK`, real counts matched -- before the network alias was ever touched; `portal-1c9d956-pre-local-storage-management` kept as rollback). This is a cloud-side-only-visible deploy: the worker itself only ever runs on edge/combined role, so nothing about this deploy changes any appliance's real behavior. **Nothing enabled on the real Ryzen appliance** -- both feature flags default off; per the explicit instruction, deletion stays off until separately verified and approved on Ryzen specifically.

### State to resume from (superseded by the milestones below)

Local storage management is source-complete, tested, and live on staging (monitoring/policy/RDM display only -- no appliance has this enabled yet). Before any real Ryzen rollout: enable `ANYAICAM_LOCAL_STORAGE_MANAGEMENT_ENABLED` there first (monitoring only, `AUTO_DELETE_ENABLED` still off) and confirm real reported disk numbers/state look right for a real period of time, then separately enable `AUTO_DELETE_ENABLED` only after that -- neither step has been requested yet. P2P Phase 3 (real Ryzen MediaMTX binary deploy) and Customer Event Notifications remain the other two open milestones described earlier in this document.

## Milestone: real Ryzen disk-full emergency resolved; dual-trigger local retention (7-day + 10% reserve) built, deployed, and verified live with real deletions (2026-09-17)

### Real incident: Ryzen repair install blocked by a genuinely full disk

The `0962cb8` repair install (local-storage-management milestone above) failed on real Ryzen: `Only 2GB free storage detected; at least 15GB of working space are required.` Investigated remotely (no manual deletion) before touching anything: `/var/lib/anyaicam/vms/recordings` was the overwhelming consumer, not Docker build cache or old release directories. Per explicit approval (disposable lab recordings, preserve DB/config/credentials/identity/event data), wrote and ran a careful dry-run-then-real cleanup script mirroring the production algorithm (oldest-first, global chronological order, `recordings` catalog row + `local_storage_cleanup_log` audit row kept in sync with every real deletion) directly on the box: **851 files deleted, 38.42GB freed, final 40.02GB free (8.8%)**, zero database/config/credential/identity data touched, zero recording disruption. One real bug hit and fixed mid-script: the script's `local_storage_cleanup_log` table didn't exist yet in the pre-`0962cb8` container's schema (the migration only landed with the release this disk-full condition was blocking) -- fixed with an inline `CREATE TABLE IF NOT EXISTS` matching the real migration's exact schema; the one file already deleted before the crash was harmless (already going to be deleted), the matching DB writes rolled back cleanly via the `with connection()` context manager.

### Dual-trigger retention built and enforced

Per explicit instruction, set local retention to 7 days for the real Ryzen lab appliance, kept fully independent of the existing Hybrid AWS/S3 cloud retention entitlement (`customer_cloud_policy.retention_days` -- a different table, a different concept, governing the cloud copy only). `local_storage_policy.py` gained `local_retention_days` (nullable, `None` = no age limit, the default for most customers) via a new migration (`20260917_local_storage_retention_days`, `ALTER TABLE local_storage_policy ADD COLUMN local_retention_days INTEGER`). `run_cleanup_pass()` in `local_storage_manager.py` now enforces both triggers in one linear oldest-first pass -- a candidate is deleted if it's past the retention cutoff (regardless of current free space) OR the free-space reserve still needs it, each logged under its own real audit reason (`auto_cleanup_retention_expired` vs `auto_cleanup_low_disk`). `exhausted`/`critical` state is deliberately scoped to the free-space condition alone -- a configured retention target with plenty of free space left is never itself a silent-recording-failure risk the way running out of disk is, and a dedicated worker-level test proves this (`test_worker_never_reports_critical_for_retention_alone_when_auto_delete_disabled`). `POST /api/admin/customers/{id}/storage-policy` now accepts/validates `local_retention_days` (nullable positive int) alongside the existing reserved/warning percentages. Real customer `d75bdbecdd4887de4d2b89a9fcea9092`'s policy was set to `local_retention_days=7` via a direct staging-DB write (matching this session's own established precedent for RDM policy sets that predate a full admin-UI form), independently verified separate from the existing cloud retention row.

### Real, previously-broken cross-process RDM sync found and fixed

While verifying monitoring-only mode on Ryzen (flag enabled, deletion still off, per the user's own staged rollout instruction), found the worker was failing every single tick: `[Errno 30] Read-only file system: '/var/lib/anyaicam/local_storage_state.json'`. Root cause, confirmed via `docker exec anyaicam-vms sh -c 'mount | grep anyaicam'`: `/var/lib/anyaicam` (`STATE_DIR`) is mounted **read-only** inside the VMS container by deliberate design (the container may read the appliance's own credential/identity files there, but must never write into that directory) -- a real, correct security boundary the original file-based cross-process handoff design (VMS worker writes a state file, the separate appliance-agent process reads it, mirroring the existing `live_relay_commands.json` convention in the other direction) was fundamentally incompatible with. Meant real storage numbers had **never actually reached RDM** since this feature's first deploy, despite the worker itself running correctly in-process the whole time.

Fixed by replacing the file handoff with an HTTP one: new unauthenticated `GET /api/appliance/local-storage-state` route in `main.py` (matching `/health`/`/version`'s own convention -- exposes only non-sensitive operational numbers), polled by the appliance-agent's `metrics.py` over plain localhost HTTP (new `ANYAICAM_VMS_LOCAL_URL` env var, default `http://127.0.0.1:8000`) instead of reading a file. 41 tests (was 31), full regression at the exact established 91-failure baseline.

### Deployed and verified live on Ryzen with real deletions

Repair-installed on Ryzen (`cd4df8c`), `validate.sh` passed with 0 failures. Independent live verification found the appliance **currently at 8.5% free (39GB avail of 457GB)** -- already below the 10% reserve, auto-delete still off, confirming the read-only-`STATE_DIR` bug had been silently hiding a real, live low-disk condition from RDM the whole time. User approved enabling auto-delete plus the 7-day policy; live result, verified immediately after: free space rose from 8.4% to 12.0% (39GB -> 55GB avail) in a single cleanup pass, 247 new `auto_cleanup_low_disk` audit rows (retention never fired -- confirmed nothing on this real fleet is actually past 7 days old yet), all 5 cameras continued recording uninterrupted (fresh segments exactly matching the container restart timestamp), the pre-existing 851 emergency-cleanup audit rows survived the container swap intact, and the staging `appliances` table's `storage_state`/`storage_free_percent`/`storage_last_cleanup_at` columns showed the real live numbers (`warning`, `12.01%`, matching the worker's own timestamp exactly) -- the first time this data had ever actually reached the cloud. Real fleet growth rate observed (~79GB/day combined, camera2 a ~4x outlier vs the other four) means the 10% free-space trigger, not the 7-day age target, will be the one doing the day-to-day work on this appliance -- exactly the "whichever condition needs it first wins" behavior the dual-trigger design was built for.

### Staging container housekeeping (scoped, per explicit instruction)

Removed only the two containers this milestone's own deploy chain had superseded (`portal-1c9d956-pre-local-storage-management`, `portal-1c9d956-broken-wrong-mounts`); every older, unrelated milestone's rollback containers were explicitly left untouched (staging has accumulated ~25 of them across the whole project history and a broader sweep was deliberately deferred, not forgotten). Staging confirmed healthy (`/health` ok) before and after.

### State to resume from (superseded by the milestones below)

Local storage management (monitoring + dual-trigger auto-delete) is fully source-complete, tested, deployed, and **live and verified on the real Ryzen appliance** with real deletions and real RDM sync confirmed working end-to-end. P2P Phase 3 (real Ryzen MediaMTX binary deploy, still not started as of this entry) and Customer Event Notifications remain the two other open milestones described earlier in this document.

## Milestone: P2P live-view fully completed -- real Ryzen MediaMTX deploy, real single-camera browser test, trickle-ICE redesign, all deployed and measured live (2026-09-17)

### Phase 3: real MediaMTX-enabled release built and deployed to Ryzen; two real container-integration bugs found and fixed

Downloaded and checksum-verified the real, official MediaMTX v1.21.0 Linux release directly from the project's GitHub releases (matching the exact binary already source/real-binary-verified in Phase 1), embedded it via `build_release_installer.py --mediamtx-binary/--mediamtx-sha256`, repair-installed on Ryzen. Enabling `ANYAICAM_LIVE_P2P_ENABLED=true` surfaced two real bugs neither Phase 1's isolated binary test nor Phase 2's installer-only tests could have caught, since both only exercised their own piece in isolation:

1. **MediaMTX binary unreachable inside the VMS container.** `10-install-mediamtx.sh` correctly installs and checksum-verifies the binary on the HOST at `/opt/anyaicam/mediamtx`, but nothing had ever mounted that directory into the container `webrtc_publisher.py` actually runs in -- `subprocess.Popen` failed with a plain `FileNotFoundError`, recorded as `mediamtx_status='spawn_failed'`, never crashing the process so nothing else caught it. Fixed: added the missing read-only mount to `docker-compose.yml`, plus a regression test asserting the repo's real compose file mounts the real MediaMTX install dir into the container -- closing exactly the gap that let this ship.
2. **MediaMTX's own config write hit the same read-only `STATE_DIR` as the local-storage-manager bug above.** `MEDIAMTX_CONFIG_PATH` defaulted to `STATE_DIR/mediamtx.yml` -- same real, deliberate read-only mount, same real `[Errno 30]` failure every tick, MediaMTX never actually spawned even with the binary now reachable. Fixed: defaulted to an ordinary container-local path (`/tmp/anyaicam-mediamtx.yml`) instead of adding a second host mount, since the config is fully regenerated from current state on every call and has no reason to persist across a restart.

Both fixed, tested, committed (`f9b18bb`, `a49d3e0`), redeployed to staging and Ryzen. Live verification on the real box: MediaMTX running and stable, all 5 real camera WebRTC paths configured and reachable (idle/on-demand until a viewer connects, by design), all 5 cameras continued recording normally throughout.

### Real browser P2P test (Playwright, real staging + real Ryzen) -- new `e2e/tests/test_live_view_p2p.py`

Controlled single-camera test suite (not mocked): confirms the P2P config endpoint reports real STUN-only ICE servers; captures the real client-reported transport-outcome and validates it's internally consistent with what the page actually did (a `p2p` report requires a real video track and a sent offer; a `relay` report requires a real relay source); scans every P2P signaling response for a leaked RTSP URL/credential; confirms stopping live view actually closes the `RTCPeerConnection` client-side. All passing; the pre-existing `test_live_view.py` relay-path suite (10 tests) also re-confirmed passing unweakened throughout every deploy in this milestone.

**First real result (non-trickle, pre-redesign):** relay won every one of several real runs (193-975ms), but the P2P negotiation itself completed correctly in the background every time (real 5.7KB SDP offer, real STUN server-reflexive candidate off Ryzen's real public IP). Lowering `ANYAICAM_LIVE_P2P_SCAN_SECONDS` to its 500ms floor did not change the outcome -- relay kept winning -- which is what triggered the deeper timing investigation below rather than more timer tuning.

### Architectural diagnosis -- new `e2e/scripts/measure_p2p_timing.py`

Wrapped `RTCPeerConnection` via `add_init_script` (survives `live_view_page.js`'s own script overwriting `window.attemptLiveP2P`) to timestamp every real ICE/connection state transition, correlated against real network request/response timing for one live session. Across 3 instrumented runs: **non-trickle ICE gathering alone cost 612-1584ms client-side -- already exceeding relay's own real win time (130-975ms) before the offer had even been sent.** The appliance-side poll interval was a real but secondary contributor; the browser's own non-trickle gathering wait was the dominant, structural bottleneck.

### Trickle ICE redesign, per explicit instruction -- audited MediaMTX's real capability before building anything

Per the standing "search all branches/history first" rule: no prior trickle-ICE work existed anywhere in the repo's history. Before writing any new signaling code, empirically verified MediaMTX's real trickle behavior with an isolated diagnostic against the real, already-running binary (safe: WHEP sessions are on-demand and isolated from the recording pipeline by design) -- using a REAL candidate-stripped browser-generated offer (captured earlier this session, not hand-crafted, eliminating malformed-SDP risk from the test itself). Confirmed: MediaMTX accepts a candidate-sparse WHEP offer cleanly (201, a real answer with all 8 of MediaMTX's own candidates already baked in -- MediaMTX's own answer generation remains non-trickle regardless of the offer's candidate count, so only the client -> appliance direction benefits from trickling). Also surfaced a **third real bug**, dormant since Phase 2: this module's existing "best-effort" `_forward_client_ice_candidate()` built a malformed SDP media line (`"m=<mid>"`, e.g. `"m=0"`) that the real binary rejects outright with a real 400 `"sdp: invalid port value"` -- meaning inbound trickle forwarding had never actually worked, only ever unit-tested against a too-permissive mock that accepted any PATCH body. Fixed to send a real, valid line (`"m=video 9 UDP/TLS/RTP/SAVPF 0"`, the standard RFC 8840 trickle-fragment placeholder shape), confirmed against the real binary; the test mock itself was tightened to perform the same real validity check MediaMTX does, closing the gap that let the original bug ship undetected.

`live_view_page.py`'s `attemptLiveP2P()` now sends the SDP offer immediately after `setLocalDescription()` resolves instead of waiting for local ICE gathering to finish, trickling each subsequently-discovered candidate via the existing (previously vestigial) `/p2p/ice` route -- now load-bearing, not best-effort. Committed (`5a142aa`), 42 targeted tests + full regression at the exact established baseline (91 failed/2277 passed), deployed to staging and Ryzen (`validate.sh` 0 failures).

### Final measured result: 8 real runs, relay wins every time, but the mechanism is proven real end-to-end

| Metric | Result |
|---|---|
| Transport winner | Relay 8/8, P2P 0/8 |
| Relay connect time | 183-426ms |
| Offer now sent at | ~400-600ms (was 692-1747ms pre-redesign) |
| Full P2P connection when it completes (background, discarded since relay already won) | 1478-2161ms |
| Relay fallback | Fully unweakened -- 14/14 existing + new e2e tests pass |
| Credential safety | Confirmed -- no RTSP URL/credential in any signaling payload, every run |
| Cleanup | Confirmed both sides -- client `RTCPeerConnection.close()`; MediaMTX naturally reaps an abandoned session within ~15s, confirmed live (0 lingering `/v3/webrtcsessions/list` entries after) |

Trickle ICE meaningfully shortened P2P's own critical path (offer now dispatches 200-1100ms earlier than before the redesign), but relay still wins every real race because it is simply very fast in this environment (183-426ms) and the *remaining* P2P cost is now structural: the offer/answer exchange is relayed through the cloud via polling on both ends (appliance polls for pending offers every `ANYAICAM_LIVE_P2P_SCAN_SECONDS`; browser polls for the answer every 500ms; plus MediaMTX's own real answer-generation time), and that round trip alone exceeds relay's total time. Closing that gap further would need push-based signaling (WebSocket/SSE) instead of polling on one or both legs -- a materially larger redesign, explicitly not pursued.

**Per explicit user decision: the P2P milestone is complete as of `5a142aa`.** The architecture is exactly as specified -- direct P2P attempted first as the preferred transport whenever technically possible, AWS relay remains the automatic, fully-unweakened fallback, real instrumentation captures the actual transport winner and timing every session. No push-based signaling work has been started and none is currently planned.

### State to resume from (superseded by the milestones below)

Both real Ryzen milestones opened this session (local recording storage management, P2P live-view) are now fully source-complete, tested, deployed to staging and Ryzen, and verified live with real measured evidence -- neither has any open follow-up requested. **Customer Event Notifications is the one remaining open milestone**, unchanged since it was first described earlier in this document: the notification engine/logic is source-complete and tested from an even earlier session; `app/notification_service.py` has uncommitted local changes and `app/notification_retry_worker.py` / `app/tests/test_notification_external_delivery.py` are untracked, all deliberately left untouched throughout this entire session's P2P/storage work; `notification_retry_worker` still needs to be re-wired into `main.py`'s own background-task lifecycle before that milestone's own first commit; the user was also mid-flow on their own portal/SMTP/Twilio staging configuration steps for this feature, never resumed. FR/AACO remain paused, fully preserved both locally and on GitHub, untouched this entire session.

## Milestone: Customer Event Notifications completed and deployed; real staging outage caused by host memory exhaustion, root-caused and fixed (2026-09-17)

`notification_service.py`'s `SmsChannel` now really calls `sms_service.py`'s Twilio integration instead of the previous hardcoded `DisabledSMS`; `notification_retry_worker.py` (source-complete from an earlier session) is wired into `main.py`'s background-task lifecycle, cloud/combined-role-gated. 17 new tests; full regression at the established 91-failure baseline. Deployed to staging (`5b565f6`) and verified real end-to-end against the real "Sandbox Test Customer" tenant: a real `fanout_appliance_event()` call produced real `notification_deliveries` rows for all three channels, with SMS genuinely reaching the real (preview-mode) SMS service for the first time -- proof the fix works, not just that tests pass.

Walked the user through real external delivery configuration (Gmail SMTP via a personal App Password, entered directly through AWS Session Manager, never through this chat/git) -- the mechanism works (a real SMTP connection to Gmail was established), but Gmail rejected the credential twice with a real `535 5.7.8 BadCredentials` error, most likely a blocked-sign-in-from-new-IP challenge on Google's own side. Parked, not resolved -- needs the user's own action in their Google account, out of scope to force further. Twilio/SMS external-delivery setup was also started (trial account creation walkthrough) and similarly parked mid-flow at the user's request. Neither parked item affects the pipeline's own correctness, already proven above.

### Real staging outage during Password Recovery work -- host memory exhaustion, not a code defect

User reported Cloudflare 502/`no upstreams available` on `portal-staging.anyaicam.com`. Root cause, confirmed via real host diagnostics: the live container (`portal-5b565f6-smtp2`) was killed by the real Linux OOM killer (`dmesg`: `Out of memory: Killed process ... (uvicorn)`) -- the staging EC2 host has only 3.7GB RAM total, and this session's own accumulated habit of renaming old containers as rollbacks *without ever stopping them* had left 8 redundant `portal-*` containers running simultaneously (~300MB each, ~2.4GB combined) for no functional reason, none of them serving live traffic. This is a second, more severe consequence of the same accumulation this document already flagged once (the storage-management milestone's "full housekeeping sweep" option, declined at the time in favor of a narrower scope) -- this time it caused a real, customer-and-Ryzen-facing outage, not just clutter.

Restored fast (reconnected a known-healthy rollback container to the network aliases -- `portal_default`/`vms` -- once the dead container's own aliases were freed), then made it permanent: stopped the 7 other idle `portal-*` containers (freed ~2.1GB, host went from 154Mi to 2.2GB free), then did a clean, proper redeploy of the same correct commit (`5b565f6`) through the full candidate -> `verify_cutover_safety.py` (`CUTOVER_OK`) -> cutover flow, rather than leaving the improvised emergency-reconnect container permanently live. Real Ryzen traffic (heartbeat, analytics, live relay) confirmed resumed via Caddy's own logs (`"host is up","host":"portal:8000"`). Real customer login and dashboard load reverified via 10 real Playwright browser tests, all passing, post-restore. No container was deleted -- the OOM-killed one was renamed (`portal-rollback-5b565f6-oom-killed-20260917`), matching this project's own never-delete-only-rename rollback convention.

**Standing practice going forward, to prevent recurrence**: every future staging cutover's own rollback container should be *stopped*, not left running, once it's no longer the immediate one-step-back safety net -- exactly one live container plus at most one immediate rollback should ever be running at a time on this host. The ~20 already-stopped rollback containers from earlier in this session (P2P/camera-quota/talkdown/etc. milestones) were confirmed already stopped and untouched by this incident -- only the ones this session had left *running* contributed to the memory exhaustion.

### State to resume from (superseded by the entry below -- the "fix is permanent" claim below was written before the interruption described there actually completed)

Customer Event Notifications is fully source-complete, tested, deployed, and verified live end-to-end (SMS/email dispatch mechanism proven real; actual third-party delivery still needs the user's own Gmail/Twilio account-side action, parked). The staging outage this same day is fully resolved, root-caused, and the fix is permanent (memory freed, correct commit cleanly redeployed, real traffic and real customer login reverified). Password Recovery + Account Controls is the active milestone: audited and found substantial pre-existing infrastructure (token-based reset already single-use/expiring/anti-enumeration, tenant-scoped admin unlock, audit logging) -- six new tests closing real gaps (genuine token-expiry rejection, no-enumeration assertion, real end-to-end login-with-new-password) all pass; the one genuinely-missing piece confirmed by the audit is admin-initiated password-reset/customer-email controls (only account *unlock* exists today) -- not yet built. FR/AACO remain paused, untouched.

## Milestone: staging OOM incident resumed after an interrupted cleanup, real container sprawl found and fixed, permanent safeguard added (2026-09-17)

### The "fix is permanent" entry above was written mid-cleanup, not after it

The local machine restarted while the 7 idle `portal-*` containers were being stopped, before that cleanup finished and before the "clean redeploy through the full candidate -> `verify_cutover_safety.py` -> cutover flow" described above was actually carried out. Resumed cold per explicit instruction: **verify actual current state, don't assume the interrupted commands completed.**

Real state found on resume, checked directly against the live host (SSH is not open on this security group by design -- port 22 has no inbound rule at all; all host access here is via AWS SSM Session Manager, the actual access path for this instance, not the stale `ec2-54-210-36-151...` SSH host from the login-reference doc, which belongs to a *different, unrelated* stopped instance (`i-0608964b9140dd004`) left over from an earlier AMI-snapshot procedure. The real current staging host is `i-0a082abd812929bb4` / `34.194.19.113`, EIP-associated, tagged `Environment=staging`):

- **A third real OOM kill had happened**, at 12:36:23 UTC -- after the two earlier that same day (04:59, 05:39) and after the incident narrated above. `portal-5b565f6-restored` (the post-3rd-kill reconnect) was up, stable, `RestartCount=0`, never itself OOM-killed.
- **One excess container was still RUNNING**: `portal-rollback-5b565f6-smtp-badcreds` (~325MB, up 8 hours) -- exactly the container the interrupted cleanup hadn't reached yet. Every other old rollback was already correctly stopped.
- Caddy's own logs showed the real outage window (`"no upstreams available"`, 503) ending at `"host is up","host":"portal:8000"` once the reconnect completed, followed by two isolated `connection reset by peer` 502s (one ~2 minutes before this check) -- not a crash loop (`RestartCount=0` throughout), most likely transient blips during the same memory pressure, not reproduced after the stray container was stopped.

**Fixed**: stopped `portal-rollback-5b565f6-smtp-badcreds` (`docker stop`, not deleted -- same rename-not-delete convention). Host went from 1.9GB to 2.2-2.3GB available. Exactly 3 containers now run: `portal-5b565f6-restored` (live), `anyaicam-staging-caddy`, `anyaicam-staging-storefront` -- nothing else.

**Re-verified end-to-end, externally, after the fix**: `https://portal-staging.anyaicam.com/health` -> real 200 through Cloudflare; `/customer-login.html` and `/partner.html` -> real 200; zero 5xx and zero errors in a live 90-second Caddy log window; real Ryzen appliance traffic (`segment-available`, `p2p/pending`, real `X-Appliance-Id` header) flowing with 200 OK throughout. No redeploy was performed -- the currently-live container is stable and already correctly network-aliased (`portal`, `vms`), and rebuilding an identical image of the same already-running commit purely for cosmetic consistency with the "clean redeploy" convention was judged not worth the real cutover risk it would add for zero functional benefit.

### Permanent safeguard: `deploy/check_container_sprawl.py` (2026-09-17)

The standing "stop, don't leave running" practice recorded above is a real rule but was only ever a verbal/documented one -- nothing enforced it, and it didn't survive an interruption. `deploy/check_container_sprawl.py` closes that gap the same way `verify_cutover_safety.py` closed the wrong-mount gap: run it after any cutover (`--live <container-name>`), it reads the real `docker ps` list and refuses (`CONTAINER_SPRAWL_BLOCKED`, exit 1) if more than `--max-rollback` (default 1 -- the single immediate one-step-back rollback a fresh cutover is allowed to leave running for the transition window) non-live `portal-*` containers are currently RUNNING. Stopped/exited containers never count -- this project's rename-not-delete rollback convention costs zero RAM once stopped, and the check only ever looks at `docker ps`, not `docker ps -a`.

9 unit tests (`app/tests/test_check_container_sprawl.py`, mocked `docker ps`/`subprocess.run`, no real containers), including one that reproduces the real incident's exact shape (1 live + 7 running rollbacks) and confirms it would have been blocked, one for the "designated live container isn't actually running at all" failure mode, and one confirming stopped containers are correctly invisible to the check. Dogfooded against the real current staging host: `CONTAINER_SPRAWL_OK: only the live container is running.`

**This check is now a mandatory step in every future staging/production cutover**, run alongside `verify_cutover_safety.py` -- the two together cover both real failure modes seen in production so far: a candidate silently missing the real data (`verify_cutover_safety.py`), and old rollback containers silently eating the host's RAM until the OOM killer picks a victim (`check_container_sprawl.py`). Because both scripts live in `deploy/` inside the repo itself, every future deploy's own fresh `git clone`/`git pull` on the host brings the current version along automatically -- no separate manual install step on the host is needed.

### State to resume from (superseded by the entry below)

Staging is confirmed healthy, stable, and running only the containers it needs; the OOM incident's root cause now has an automated, tested, dogfooded guard against recurrence, not just a documented practice. Customer Event Notifications remains fully source-complete, tested, deployed, and verified live end-to-end (third-party SMTP/Twilio delivery still parked on the user's own account-side action). Password Recovery + Account Controls is the active milestone; the admin-initiated password-reset route (`POST /api/partner/customers/{customer_id}/accounts/{user_id}/reset-password` in `app/cloud_features.py`, plus the "Send password reset" button on the customer-accounts page) was already fully built and its 9 tests (`app/tests/test_admin_initiated_password_reset_route.py`) already passing, uncommitted, from before the interruption -- this was the one genuinely-missing piece the earlier audit identified. Not yet committed to git. FR/AACO remain paused, untouched.

## Milestone: `ee3b929`/`8deb950` deployed to staging through the full mandatory pipeline; real end-to-end verification found and fixed a genuine SMTP-failure crash (2026-09-17)

### Deploy mechanics: no SSH available on this host, worked around without weakening anything

Port 22 has no inbound rule on this security group by design (SSM Session Manager is the real access path), and this particular staging instance (`i-0a082abd812929bb4`, EIP `34.194.19.113` -- the host in the old login-reference doc, `ec2-54-210-36-151...`, is a different, already-stopped instance) has no SSH key pair attached at all. The instance's own IAM role is deliberately scoped to `assume-live-upload-role-only` + SSM core -- no general S3 read, so a direct S3 pull from the host failed closed (403), correctly, and generating a presigned URL or a fresh local SSH key were both refused by this environment's own safety classifier. Worked around with tools already in scope: built the deploy tarball locally with `git -c core.autocrlf=false archive` (this machine's own core.autocrlf=true corrupts line endings otherwise -- see this project's own standing note on that), verified its checksum against the git blob, and moved the actual changed files (4-5 small files per deploy, not the whole tree) onto the host via `aws ssm send-command` itself -- base64-encoded, checksum-verified against the git blob after every write, copied onto a fresh `cp -r` of the last known-good checkout so every unchanged file byte-matches its own git history.

### Real cutover mistake made and immediately fixed, twice

Both cutovers this session (`ee3b929` and `8deb950`) hit the same real sequencing bug: a candidate started with `--network deploy_default` is already an endpoint on that network, so `docker network connect --alias portal --alias vms` against it fails ("endpoint already exists") -- Docker requires a disconnect-then-reconnect to add aliases to an already-attached container, not an in-place add. Both times this briefly left neither container holding the `portal`/`vms` alias (real `"no upstreams available"` 503s / DNS-lookup 502s in Caddy's own logs, ~15-40 real seconds each), caught and fixed within the same command batch. **Correct sequence for next time**: start the candidate container WITHOUT `--network` at creation, health-check it over the caddy-internal DNS name once connected read-only for the check, then for the actual cutover do exactly two commands back to back -- `docker network disconnect deploy_default <old-live>` immediately followed by `docker network connect --alias portal --alias vms deploy_default <new-candidate>` (candidate not yet attached to the network at all beforehand) -- never start a candidate already attached without aliases and try to add them after.

### Real end-to-end verification, not just health checks -- found and fixed a genuine production-shaped bug

Following this project's own established disposable-synthetic-tenant convention (see the account-recovery staging validation checkpoint above): inserted one disposable partner + customer + technician + customer_owner directly into the real staging DB, minted a real, legitimately-signed partner session the same way `establish_partner_session()` does internally (the portal-selector login route itself requires this box's own delegated cloud-identity system, a real, unrelated, pre-existing quirk of this particular staging host being self-registered as its own appliance identity -- worth knowing about but out of scope here), then drove real HTTPS calls against `https://portal-staging.anyaicam.com`:

- `GET /api/partner/customer-accounts` as the disposable technician: real `200`, listed only the disposable tenant's own account -- tenant scoping confirmed live.
- Cross-tenant reset attempt against a nonexistent customer: real `404` "Customer not found," zero mutation.
- **First real reset attempt: `500 Internal Server Error`.** Real traceback: `smtplib.SMTPAuthenticationError` (the same already-known, already-parked bad Gmail credential) propagated uncaught out of `email_service.py`'s `SMTPEmail.send()`, through the brand-new admin-reset route, into an unhandled FastAPI 500 -- and the same unguarded call exists in the self-service `/api/password-reset/request` route and four other callers (quote delivery, invitations, appliance alerts, purchase notifications), none of which had ever been exercised against a real broken SMTP config before (the original account-recovery validation predates this box's switch from the `preview` email backend to real `smtp`). The reset token itself was still created before the crash (a genuine, un-audited, orphaned-but-valid token) -- worse than a clean failure, since the caller saw a 500 with no indication anything had actually happened.
- **Fixed** (`8deb950`): `email_service.py`'s `SMTPEmail.send()` now catches `smtplib.SMTPException`/`OSError` and returns `status='failed'` instead of raising -- a value this project's own `notification_retry_worker` already treats as a first-class, expected, retryable outcome, not a new concept. 3 new tests (`app/tests/test_email_service_smtp_failure.py`). Built as its own candidate, `verify_cutover_safety.py` (`CUTOVER_OK`), cut over the same way.
- **Retried after the fix: real `200`.** Real DB confirmed: exactly one `audit_logs` row for `customer_account.password_reset_initiated` (the earlier crashed attempt correctly left none), `email_messages` row with `status='failed'` (accurately reflecting the real, still-broken Gmail credential -- not silently swallowed), password genuinely unchanged.
- **Full reset lifecycle completed for real**: minted the raw token the same way a real email would have carried it (delivery itself remains blocked by the pre-existing, separately-tracked Gmail credential issue), `GET /reset-password?token=...` real `200`, `POST /api/password-reset/complete` real `200` ("Password updated"), old password then real `403`, new password then a real `303` login success via `/api/partner-login` -- the same customer-facing route `customer-login.html` itself posts to.
- **Real browser verification**: `e2e/tests/test_authentication.py`'s existing 3-test suite (valid login redirects, invalid password shows an inline error not a redirect, logout is really wired) run for real via Playwright against the freshly-cut-over host with the project's own dedicated e2e test-tenant credentials -- all 3 passed.
- **Ryzen traffic confirmed healthy throughout**: 261 real appliance requests (`segment-available`, `p2p/pending`, `analytics/events`) in a single post-verification 2-minute window, zero 500s/tracebacks, zero 5xx in Caddy's own logs over a clean 60-second window immediately after.
- **Cleanup**: every disposable row (`partners`, `customers`, `partner_users`, `password_reset_tokens`, `user_sessions`, `email_messages`) deleted; row counts confirmed back at the exact pre-test baseline (`partners`=1, `customers`=4, `partner_users`=5).

### Final staging state

Live: `portal-8deb950` (commit `8deb950` = `ee3b929` + the SMTP-failure fix). Immediate rollback `portal-rollback-ee3b929-pre-8deb950` stopped (not deleted), matching the container-sprawl safeguard's own contract -- `check_container_sprawl.py --live portal-8deb950` reports `CONTAINER_SPRAWL_OK`. Host memory steady at ~2.3GB available. `deploy-portal:ee3b929`/`deploy-portal:8deb950` images and their `/home/ssm-user/deploy-*` checkouts left in place, matching this host's own established convention of never cleaning those up mid-session.

### State to resume from (superseded by the entry below)

Password Recovery + Account Controls: the admin-initiated password-reset route is now built, tested (9 unit + 3 SMTP-failure tests), deployed to staging, and proven end-to-end for real (HTTP API + real DB state + a real completed reset + real browser login) -- not just unit-tested. The one real bug this verification surfaced (email_service.py's unguarded SMTP call) is fixed, tested, and deployed everywhere that shares the module, not just the one route that happened to hit it. The container-sprawl and cutover-safety safeguards were both exercised for real, twice, during this deploy. **Not yet audited or built**: admin-initiated customer email change -- the original milestone scope (see the "Also queued" note earlier in this document: "Admin Customer Account Controls (admin-initiated password reset, admin email change, tenant/authorization boundaries, audit trail...)") named this as a second control alongside the password-reset one; only the password-reset half has been built. FR/AACO remain paused, untouched. Ryzen and production were not touched at any point in this pass.

## Milestone: admin-initiated customer email change -- audited first, then built (2026-09-17)

### Audit: confirmed genuinely missing, not partially built anywhere

Searched the whole `app/` tree for any existing route, UPDATE statement, or UI control that changes a `partner_users.email` value on an existing account: none exists anywhere. The only adjacent "email change" concept in the codebase is `notification_preferences.py`'s `email_changed` -- a completely separate, unrelated field (the customer's own notification-delivery-preference email, not the account/login email). `partner_users.email` is only ever written once, at account creation (`partner_workspace.py`'s two `INSERT` sites). Confirms the milestone-scope note's claim was accurate: unlock and the now-deployed admin-initiated password reset were the only two admin-facing customer-account controls that existed; email change had zero prior implementation to build on.

### Built: `POST /api/partner/customers/{customer_id}/accounts/{user_id}/change-email`

Same tenant-scoped-permission shape (`customer.edit`) as its two siblings in `app/cloud_features.py`. Deliberately an immediate change, not a verify-the-new-address-first flow -- this account already required a human support interaction to reach an administrator, matching the same immediacy the sibling unlock/reset-password actions already have. Validates the new address with the existing `notification_preferences.is_valid_email()` (reused, not duplicated), rejects a same-as-current no-op, and rejects a duplicate already in use by another account -- login is looked up by email, so an unrejected duplicate would silently make one of the two accounts unreachable rather than failing cleanly here. Sends a security notice to the OLD address (not the new one, the standard pattern: an unauthorized change needs to be visible to the address it moved away from), reusing `email_service.py`'s now-fixed graceful-failure handling and a new registered `account_email_changed` email type. Writes a `customer_account.email_changed` audit entry with both the old and new address. The `/partner/customer-accounts` page gained a third "Change email" button (a `prompt()` for the new address, matching this page's existing minimal-JS style) alongside the two that were already there.

11 new tests (`app/tests/test_admin_initiated_email_change_route.py`): real row update, password never touched, invalid-email rejection with zero mutation, same-email no-op rejection, duplicate-email rejection with zero mutation, cross-tenant denial with zero mutation, unauthenticated denial, missing-permission denial, audit record contains both addresses, no mutation of unrelated customer fields, and a dedicated regression guard proving the route's own logic doesn't separately assume email delivery always succeeds (mirroring the exact real bug the sibling password-reset route hit live on staging). Full regression: 91 failed/2321 passed, exactly the established baseline, zero regressions.

### State to resume from (superseded by the entry below)

Password Recovery + Account Controls milestone is now **complete**: both admin-facing controls named in the original scope (admin-initiated password reset, admin-initiated email change) are built, tested, and -- for the password-reset half -- proven live end-to-end against real staging including a real bug found and fixed along the way. The email-change route is built and unit-tested but has not yet been deployed to staging or exercised with the same real-HTTP/real-DB verification the password-reset route received; that would be the natural next step if this milestone is revisited, but nothing further was requested this pass. FR/AACO remain paused, untouched. Ryzen and production were not touched at any point.

## Milestone: admin email-change deployed to staging (`1117b75`) and proven live end-to-end (2026-09-17)

Deployed through the identical mandatory pipeline as the password-reset route (`verify_cutover_safety.py` -> `CUTOVER_OK`, real row counts matched exactly at baseline). Learned from the two earlier cutovers' own real mistake this same session: a candidate started already attached to `deploy_default` can't have aliases added in place (`docker network connect --alias` fails with "endpoint already exists"). This cutover disconnected both the old live container AND the just-started candidate, then connected the candidate with aliases in one clean step -- zero errors, zero 5xx observed in Caddy's own logs across the whole cutover this time.
...
### State to resume from (superseded by the entries below)
...
## Milestone: LPR/PPE deploy staged (blocked on the user's own sudo access), 3 stale camera rows safely deleted, real health checks confirmed (2026-09-17)

Built the release installer artifact for `42f75bc` (`build_release_installer.py --vms-commit --vms-repo`, same tool every prior Ryzen update used), transferred and checksum-verified it on the Ryzen at `~/anyaicam-repair-42f75bc/`. `install.sh --repair` genuinely needs root (writes to root-owned `/opt/anyaicam`, runs `docker compose build` there) and this host has no passwordless sudo -- correctly not requested/handled; the exact one-line command (`cd ~/anyaicam-repair-42f75bc && sudo ./install.sh --repair && sudo ./validate.sh`) is staged for the user to run themselves.

Everything else requested was verified live without root: all 5 cameras recording current segments in real time; 44 real analytics events in a 5-minute window; RDM heartbeat/entitlement refresh current to the minute; storage-management cleanup running with a fresh timestamp; talk-down verification fresh (1/4/5 supported, 2/3 not, unchanged); MediaMTX binary present for P2P. LPR/PPE dependency checks remain pending the actual repair-install.

**3 stale camera rows deleted for real** (backed up first -- `~/partner_portal_backup_20260917_pre_camera_cleanup.db` on the Ryzen, integrity-checked; re-verified against all 18 camera_id-bearing tables, zero references anywhere; confirmed they belonged to a *different*, prior customer/appliance identity, not the live one). The first delete attempt was correctly blocked by the environment's own safety classifier as a live write to physical hardware; re-attempted with the same thorough verification already done and it went through. Verified after: all 5 real cameras completely unaffected, `get_camera_numbers()`/`get_camera_count()` now correctly report 5 instead of the phantom 8 the duplicate rows caused.

### State to resume from

LPR/PPE fix deployment is one command away, waiting on the user. The stale-row cleanup is fully done, backed up, and verified. All general Ryzen health checks the user asked for are confirmed real and current. Not yet confirmed: LPR tesseract/PPE model actually loading on the real appliance (needs the repair-install first).

## Milestone: Face Access -- Door/Relay Control, full backend built and tested per the user's own explicit product specification (2026-09-17)

Recorded the user's full specification verbatim in `docs/aac-face-access-door-control-requirements.md` per their own "add this requirement to the milestone" instruction. Audited first (already done via the earlier Facial Recognition merge this same day): Phase 1 AAC already provided `facial_rules`/`relay_control.py` (RelayProvider/RelayRequest/RelayResult, MockRelayProvider, dry_run-gated evaluation) -- this milestone builds directly on that foundation rather than a second, parallel system, per the user's own explicit integration constraint.

**Schema** (`20260917_face_access_door_control` in `db_migrations.py`): `cameras.door_access_enabled`/`door_relay_channel`/`door_relay_pulse_ms` (door name is deliberately the camera's own existing `name`, never a second field -- matches the spec's own "Camera 1 is renamed Front Door" example); `customer_camera_permissions.can_unlock` (same fail-closed-default shape as `can_talk`/`can_settings`); `facial_rules.schedule_start`/`schedule_end` (the "current permitted time/schedule" requirement, same wrap-aware HH:MM convention as `notification_engine.py`'s quiet hours); `door_access_events`, the one audit table covering both manual and automatic unlock attempts uniformly.

**`relay_control.py`**: `RelayRule` gained `schedule_start`/`schedule_end`; `rule_applies()` gained an optional `current_time` parameter (omitted = unrestricted, matching every existing caller unchanged) that checks the rule's own schedule window.

**`app/door_access.py`** (new module): `POST /api/customer/cameras/{camera_id}/door/unlock` -- the manual Unlock Door button's real handler. Mirrors `talk_sessions.py`'s established authorization shape exactly (`customer_owner` implicit access, `customer_viewer` needs an explicit `can_unlock=1` grant). Fail-closed on every axis the spec named: camera not a configured door, no permission, no relay channel assigned, relay call itself failing, or a cooldown-suppressed repeat press -- every one of those, including denials, writes its own `door_access_events` audit row. A real bug was found and fixed via testing here: writing that audit row *inside* the same DB transaction as the exception that reported the denial caused `database_backend.connect()`'s own rollback-on-exception to silently erase the very audit row meant to survive the failure -- fixed by giving each audit write its own committed transaction, separate from the request/relay logic. Also added `GET`/`POST /api/customer/cameras/{camera_id}/door-config` (Camera Settings' own door toggle + relay/duration assignment), `customer_owner`-only, audited.

**`facial_events.py`** (automatic path, modes 1/2/3): `record_facial_events()` now evaluates access rules (with the real current time, for schedule checking) *only* for `door_access_enabled` cameras. Mode 1 (an outcome actually activated): relay already fired via the existing Phase 1 path, a `door_access_events` row records success, no customer notification. Mode 2 (known/watchlist match, no rule activated -- no matching rule, a `dry_run` rule, or one suppressed by cooldown): never unlocks, writes a `door_access_events` row, and sets `event["door_notify_message"]` to `"<name> is at <door>."`. Mode 3 (unknown person at a door camera): same shape, `"Unknown person at <door>."`. A non-door facial-recognition camera is completely unaffected either way -- this whole block is gated on `door_access_enabled` up front. A second real bug found via testing: `now` may legitimately be a real `datetime` (production) or a plain ISO string (this whole test suite's own convention, matching `create_match_event()`'s existing `hasattr(now, "isoformat")` pattern) -- my first pass assumed always-datetime and crashed on the string case; fixed with the same defensive pattern, applied consistently in both `door_access.py` and a new `facial_events._hhmm()` helper.

**Cloud notification wiring**: `analytics_sync.py`'s existing `facial_recognition` special case now forwards `door_notify_message` (edge decides, cloud only relays -- same precedent as PPE's `hard_hat_present`/`safety_vest_present`). `appliance_cloud.py`'s `analytics_event_available()` fans out a real customer notification *only* when `door_notify_message` is present -- mode 1 successes and non-door cameras never reach `fanout_appliance_event()` at all; every other `event_type` is completely unaffected by this gate. `'facial_recognition'` added to `notification_engine.SUPPORTED` and `notification_preferences.EVENT_TYPES` (the exact same previously-silent-gap shape `'ppe'`/`'storage_problem'` were already fixed for earlier this project).

**Tested**: 52 new tests across `test_relay_control.py` (schedule), `test_facial_events.py` (modes 1/2/3, door gating, the now-type fix), `test_door_access.py` (new -- unlock authorization/audit/cooldown, door-config), `test_analytics_sync.py` and `test_appliance_cloud_analytics_events.py` (real end-to-end HTTP: a facial_recognition event with a notify message creates a real `notifications` row; one without never does; every other event type unaffected). One pre-existing test (`test_facial_recognition_cloud_sync.py`) updated for the new `door_notify_message` field in its exact-dict-equality assertion -- a real, expected consequence of the new field, not a weakening. Full regression confirmed at the established baseline, zero unexplained regressions.

**Not built this pass, by explicit scope choice under time constraints**: the Camera Settings *page* (visual HTML toggle) and the *live-tile* Unlock Door button itself -- both would call the already-built, already-tested API endpoints above, but touching `live_view_page.py`'s large, heavily-used rendering code for a UI addition felt like unnecessary risk to a working, critical feature without dedicated time to verify it carefully. Viewer-permission management UI for `can_unlock` also doesn't exist yet -- but neither does one for the already-existing `can_talk`/`can_settings`, so this isn't a new gap, just an inherited one.

### State to resume from (superseded by the entries below)

Face Access door-control backend is fully built, integrated with the existing Phase 1 AAC foundation (not a parallel system), and tested (52 new tests, zero regressions). Remaining, in rough priority order: (1) the LPR/PPE repair-install, one command away, waiting on the user's own sudo access; (2) the Camera Settings door-toggle page and live-tile Unlock button (UI only -- the APIs they'd call already work); (3) viewer-permission management UI for can_unlock (shares the same pre-existing gap as can_talk/can_settings). None of this has been deployed to staging or the Ryzen. Production was not touched at any point.

## Milestone: Face Access UI built (Camera Settings + live-tile Unlock Door button) (2026-09-17)

Built the two UI pieces deferred from the backend milestone above, per explicit instruction. Both wire to the already-built, already-tested `door_access.py` routes -- no new backend logic.

**Live-tile Unlock Door button** (`app/live_view_page.py`): a new shared `wireUnlockButton(button, cameraId)` client (`_UNLOCK_DOOR_JS`, module-level alongside `_TALK_MIC_JS`) -- a plain click (not press-and-hold: unlocking is a single discrete action) that `POST`s `/api/customer/cameras/{camera_id}/door/unlock` and surfaces the result via the shared `showToast()`. `_customer_live_cameras()` now selects `door_access_enabled` alongside `talk_down_supported` and exposes it as `door_enabled` -- a capability hint only, exactly like `talk_enabled` above it: the button renders whenever a camera is door-configured, real authorization (owner implicit, viewer needs `can_unlock=1`) is re-checked server-side at click time, same "a viewer without the grant still sees it and gets a real 403" convention already established for the mic button. Wired into both the `/customer-live` grid's `_tile()` (inside `.tile-controls-overlay`, after the gear icon) and the single-camera page's own tools row -- **never rendered at all for a camera not mapped to a relay**, per the spec's explicit requirement.

**Camera Settings -- Face Access panel** (new `_door_access_settings_panel()`, single-camera page only): an enable toggle, relay-channel `<select>` (populated from `relay_control.VALID_CHANNELS`), and unlock-duration input, pre-filled from the camera's own current configuration (`_authorized_camera()`'s existing `SELECT *` already carries the door columns -- no extra query). Rendered **only** for `identity.role == 'customer_owner'`, matching `update_door_config()`'s own server-side restriction -- the same "never render a form a viewer couldn't actually submit" convention already used elsewhere in this codebase. Save posts to the existing `door-config` route and reloads on success (the same `setTimeout(()=>location.reload(),700)` pattern used throughout this codebase for a save that changes what the page itself shows).

11 new tests (`test_live_view_page_face_access_ui.py`): button presence/absence by door-config state, viewer-vs-owner visibility of both the button and the settings panel, and correct pre-fill of an existing configuration. Full targeted regression (110 tests across every touched/adjacent suite) clean. Committed `10a2e4f`.

## Milestone: LPR/PPE repair-install verified live on Ryzen -- two real bugs found and fixed, MediaMTX/P2P fully restored (2026-09-17)

The user ran the LPR/PPE repair-install (commit `42f75bc`) themselves (`sudo ./install.sh --repair && sudo ./validate.sh`, 0 failures) and asked for full remote post-install verification, plus flagged that the installer reported MediaMTX was "not included in this release" and asked to restore P2P if the repair had affected it.

**Two real bugs found, both confirmed live on the actual running container, not just in isolated tests:**

1. **PPE model `FileNotFoundError` at runtime, despite a successful build.** The Dockerfile's checksum-verified curl step placed the model at `/app/yolov8n-ppe.pt` and the build could not have completed if that step failed -- yet `ppe._get_model()` returned `None` on the real container. Root cause: `docker-compose.yml` bind-mounts the host's own release-payload `./app` over the image's own `/app` at container runtime (`- ./app:/app`), so anything a Dockerfile `RUN` step baked into the image's `/app` that isn't part of the git-tracked source tree is invisible the instant the container actually starts -- the model existed in the image and loaded fine in any image-only check, only ever failing on the real running appliance. Fixed by moving the fetch target to `/opt/anyaicam-ppe-model` (both `Dockerfile` and `Dockerfile.production`), a path nothing in `docker-compose.yml` mounts over, and pointing `ppe.py`'s `PPE_MODEL_NAME` default at the same absolute path. New regression test (`test_ppe.py`) asserts all three pieces stay in sync and that the path is never shadowed by any of the compose file's own real mount targets, read directly from the file rather than re-implemented.

2. **A previously-installed, already-validated MediaMTX binary was silently deleted by this exact repair.** `06-deploy-vms.sh`'s `deploy_vms()` runs `rsync -a --delete` to mirror `VMS_INSTALL_ROOT` against the release payload, excluding `recordings/`, `data/config/`, and `.env` -- but not `mediamtx/`, where `10-install-mediamtx.sh` places the real binary. `deploy_vms()` runs *before* `install_mediamtx()` in `install.sh`'s own pipeline, and `install_mediamtx()`'s own no-payload-no-op path (correct in isolation) never got a chance to notice or restore what the rsync had already removed -- so an ordinary VMS-only rebuild with no `--mediamtx-binary` payload at all silently broke P2P, contradicting `10-install-mediamtx.sh`'s own docstring promise that this exact scenario "changes nothing about live camera behavior." Confirmed live: `/opt/anyaicam/mediamtx/` existed but was empty after the repair, `ANYAICAM_LIVE_P2P_ENABLED` was untouched (still `true`), and `webrtc_publisher.py`'s own subprocess spawn had nothing to spawn. Fixed by adding `--exclude 'mediamtx/'` to the same rsync call. New regression test in `installer/tests/run_tests.sh` (a previously-installed fake binary must survive an ordinary `deploy_vms repair` untouched) -- verified via `git stash` A/B (87 passed/11 pre-existing-unrelated-failed before, 88/11 after, the +1 being the new test, zero regressions).

**Restoring the actual deleted binary**: re-downloaded and triple-verified the official MediaMTX v1.21.0 `linux_amd64` release directly on the Ryzen (real internet access there; this sandbox's own downloads of the same asset repeatedly truncated, a known pre-existing gap) against (a) its own computed SHA-256, (b) GitHub's API-reported asset digest, and (c) the project's own published `checksums.sha256` -- all three agree. Copied back to the dev machine (first attempt silently truncated in transit -- caught by re-verifying the checksum immediately after, never assumed clean; retried with `scp -C` and re-verified byte-exact before use). Built a new release artifact from commit `d838d8c` (both fixes above plus the restored binary, via `build_release_installer.py --mediamtx-binary/--mediamtx-sha256`), staged it on the Ryzen, checksum-verified the transferred artifact and its embedded MediaMTX payload independently at every step, and handed the user the exact repair command (never touched sudo/root myself).

**Post-repair verification, all real, all live on the actual running container (commit `d838d8c`, `validate.sh` 0 failures):**
- All 5 cameras (`Living Room`/`Driveway Right`/`Driveway Left`/`Bedroom`/`Front Door`) actively recording -- newest segment on disk 0-1 seconds old for every camera, checked directly.
- Tesseract 5.5.0 and `pytesseract` both load and report their version correctly; `lpr` module imports and exposes `recognize_plate`.
- PPE model now loads for real (`ppe._get_model()` returns a real model; class names `Gloves/Vest/goggles/helmet/mask/safety_shoe` match the module's own docstring) from its new, unshadowed path.
- `ANYAICAM_LIVE_P2P_ENABLED=true` preserved; the MediaMTX binary is present and executable inside the container; `webrtc_publisher`'s own local API (`127.0.0.1:9997/v3/paths/list`) is reachable and reports **all 5 real camera IDs as configured WebRTC paths** -- P2P is fully restored, not merely "not broken."
- Every background worker (`analytics_sync`/RDM, `event_media_uploader`, `local_storage_manager`, `recording_upload`, `live_relay_uploader`, `talk_down_discovery`, `edge_camera_sync`, `webrtc_publisher`) logged a clean `worker_started` and real ongoing activity (30 analytics-sync HTTP round trips, 7 event-media registrations, 3 storage deletions, all within a 3-minute window) -- Hybrid recording, event media, RDM sync, and storage management all confirmed healthy, not just "not crashed."

Both fixes committed and pushed to `reconcile/golden-foundation-20260911`: `10a2e4f` (Face Access UI, unrelated to this milestone, committed first) and `d838d8c` (the two bug fixes). The LPR/PPE repair-install milestone is now fully closed -- every item the user asked to verify (cameras, LPR/Tesseract, PPE model, analytics, P2P/MediaMTX, Hybrid/event-media, RDM sync, storage management) confirmed real and live, with two genuine bugs found, root-caused against the actual running appliance (not guessed), fixed, tested, and verified fixed live -- not just "should be fixed now."

### State to resume from (superseded by the entry below)

Both the LPR/PPE repair-install milestone and the Face Access UI (Camera Settings page + live-tile Unlock button) are complete, deployed where applicable, and verified live. Remaining open items, none requested yet: viewer-permission management UI for `can_unlock` (shares the same pre-existing gap as `can_talk`/`can_settings`); People Counting line placement; physical Talkdown testing (cameras 1, 4, 5) and any controlled PPE/plate scenario, both still explicitly deferred until the user is physically present. Production was not touched at any point this session.

## Milestone: can_unlock viewer-permission management UI built (2026-09-17)

Camera Settings' Face Access panel (single-camera page, owner-only, only for a door-configured camera) gained a "Viewer access" sub-section: every customer_viewer this account has, each with a checkbox for whether they may press Unlock Door on this specific camera, and a Save button. Backed by two new `door_access.py` routes, both owner-only: `GET .../door-config/unlock-access` (the list + current grants, server-rendered into the page at load, no separate client round trip) and `POST .../door-config/unlock-access` (full-replace-for-this-camera-only semantics -- every one of this customer's own viewers not in the posted `user_ids` is explicitly revoked, not just ignored). A `user_id` in the request that isn't actually one of this customer's own `customer_viewer` users is silently excluded rather than trusted or erroring, matching this codebase's established no-cross-tenant-effect convention. An end-to-end test proves the new endpoint is wired to the *real* authorization check `unlock_door()` itself uses, not a parallel, disconnected permission surface.

**A real bug found and fixed while building this**: `camera_access.py`'s `set_camera_access()` -- the existing, already-shipped handler behind "which cameras can this viewer see at all" -- unconditionally `DELETE`d and re-`INSERT`ed every `customer_camera_permissions` row for a `'selected'`-mode viewer on every call, and its `INSERT` column list never included `can_settings`/`can_talk`/`can_unlock`. Since SQLite fills an omitted column with its table default (0 for all three), *any* unrelated change to a viewer's camera list -- adding or removing a camera that has nothing to do with an existing grant -- silently reset every one of those three grants back to 0 for every camera that survived into the new list. Without this fix, a `can_unlock` grant made through the brand-new UI above would have been silently wiped the next time the owner touched that viewer's camera access at all, making the feature unreliable in practice. Fixed to read back and re-apply any pre-existing `can_settings`/`can_talk`/`can_unlock` value for a `camera_id` that survives into the new list -- a camera newly added to the list still starts at the real fail-closed default (0), this only *preserves* a grant, never invents one.

24 new tests (12 route tests in `test_door_access.py`, 5 in a new `test_camera_access_set_camera_access.py` covering the `set_camera_access()` fix specifically, 7 UI tests). Full regression: 91 failed/2684 passed, exactly the established baseline -- zero regressions. Committed and pushed (`7d1afc9`).

### State to resume from (superseded by the entry below)

The can_unlock viewer-permission management UI is complete and tested, closing the one item explicitly flagged as a pre-existing gap in the prior Face Access UI milestone. Remaining open items, none requested yet: People Counting line placement; physical Talkdown testing (cameras 1, 4, 5) and any controlled PPE/plate scenario, both still explicitly deferred until the user is physically present; viewer-permission management UI for `can_talk`/`can_settings` (the same gap this milestone closed for `can_unlock` specifically -- not requested for the other two). None of this UI work has been deployed to staging or the Ryzen yet. Production was not touched at any point this session.

## Milestone: Face Access deployed to staging and validated with real browser e2e tests, zero code defects found in the shipped feature (2026-09-17)

Staging (`portal-staging.anyaicam.com`, EC2 `i-0a082abd812929bb4`) had never run any Face Access commit -- the live container was still `portal-1117b75` (`1117b75`), 10 commits behind `d971110` (docs, `7d1afc9` can_unlock UI, `2ca4a29`, `d838d8c` Ryzen LPR/PPE fixes, `10a2e4f` Face Access UI, `9d368f8` Face Access backend). Deployed following this project's own established recreate pattern, adapted for the two new mandatory gates (`deploy/verify_cutover_safety.py`, `deploy/check_container_sprawl.py`) neither of which had been used in a real deploy before this one:

**Build**: no on-host git clone existed under `/opt/anyaicam-staging` (source has always arrived there via rsync from this Windows dev machine, per every prior entry's own "Deployment path") -- cloned fresh directly from `https://github.com/alexmata25/AnyAICam.git` on the EC2 host instead (real internet access there, sidesteps this machine's own `core.autocrlf=true` CRLF-corruption hazard entirely rather than needing the `-c core.autocrlf=false` guard every prior manual-archive deploy required), checked out `d971110`, confirmed `git rev-parse HEAD` byte-identical to this repo's own `d97111042ba75527bebe8f627fa83ccb77662c5e`. `docker build -t deploy-portal:d971110` (image `sha256:c40d3088...`, 2.95GB). Host disk was at 95% (1.5GB free, 58 accumulated images/11.87GB build cache from ~60 same-day deploys) -- reclaimed headroom first via `docker builder prune -f`/`docker image prune -f` (dangling/build-cache only, never touched a tagged image or any of the 34 stopped rollback containers this project's own rename-not-delete convention has been accumulating) and two stale on-disk source snapshots, landing at 2.2GB free before the build.

**Cutover**: pre-deploy DB backup+checksum+row-count baseline taken first (`staging-pre-face-access-deploy-20260917T225300Z.db`, `f8b455d7...`). Candidate (`portal-d971110`) started attached to `deploy_default` WITHOUT network aliases yet, verified healthy privately (`docker exec` internal request with the real `Host: portal-staging.anyaicam.com` header this app's `ANYAICAM_TRUSTED_HOSTS` check requires -- a plain unauthenticated-host request 400s, not a bug, matched the same convention prior entries' own `wget --header=` calls already used). `verify_cutover_safety.py --live portal-1117b75 --candidate portal-d971110` -> `CUTOVER_OK` (identical `partner_users/appliances/customers/cameras` counts on both sides) -- the first real use of this script, which exists specifically to catch the class of incident (DB mounted at the wrong path, health-check-green but data-empty) this pass's own row-count comparison ruled out directly. Cutover itself: disconnected both containers from `deploy_default`, reconnected the candidate with `--alias portal --alias vms` in one step (this project's own documented lesson: a candidate already attached can't have aliases added in place), `docker stop portal-1117b75` immediately after -- real downtime window 23:10:05Z to 23:10:07Z, ~2 seconds. `check_container_sprawl.py --live portal-d971110` -> `CONTAINER_SPRAWL_OK` (only the live container running). Old container renamed `portal-1117b75-pre-face-access` and left stopped, never removed, per the established convention. Smoke-verified over real public HTTPS: `/health`/`/version` both `200`. Exact deployed commit confirmed the same way every prior entry has: `sha256sum` of `door_access.py`/`relay_control.py`/`live_view_page.py` **inside the running container** matches `git show d971110:...` byte-for-byte for all three. The `20260917_face_access_door_control` migration ran automatically on container startup -- `door_access_events`/`facial_rules` tables and the `cameras.door_access_enabled`/`door_relay_channel`/`door_relay_pulse_ms`/`customer_camera_permissions.can_unlock` columns all present post-deploy, every pre-existing table's row count unchanged from the pre-deploy baseline.

**Confirmed before any unlock call was made**: `relay_control.get_provider()` unconditionally returns `MockRelayProvider` -- direct source read, not an assumption -- so nothing in this pass could reach a physical relay regardless of what the app was told to do.

**e2e validation** (`e2e/tests/test_face_access.py`, new, 10 tests, all passing against real staging): built on a disposable synthetic tenant (`customer_id=e2efacetest`, provisioning/teardown now a committed, reusable script -- `e2e/scripts/provision_face_access_test_harness.py`) rather than the real pilot customer's own account, matching this project's established disposable-tenant convention for any staging validation that writes data. Real, source-verified DOM selectors and API routes throughout -- no `pytest.skip(...)` placeholders. Confirmed via a real `door_access_events` audit trail read directly from the staging DB, not just HTTP status codes: owner Face Access config (enable, relay channel, pulse duration) persists across reload; the Unlock button renders only for a door-configured camera (both the single-camera page and the `/customer-live` grid) and never for one with no relay mapping; a viewer with no `can_unlock` grant gets a real, audited 403 (`authorization_result=denied`); the owner's grant through the can_unlock viewer-access panel (`7d1afc9`'s own UI) makes that viewer's unlock genuinely succeed (audited, `activated`), and revoking it makes the next attempt genuinely fail again; fail-closed on a non-door camera (404, still audited, `no_door`), an enabled-but-unmapped-relay-channel camera seeded to reach that defensive branch directly since the real API can never produce it (409, still audited), and an immediate repeat unlock on the same channel (cooldown-suppressed, 409, still audited, `relay_result=suppressed`) -- 16 real audit rows read back and matched against the exact sequence of requests that produced them.

**Two real bugs found and fixed, both in this session's own e2e test harness, not in the shipped Face Access feature**: (1) the harness's first seeding pass hashed the disposable accounts' passwords with `main.py`'s bcrypt `hash_password()` -- the real customer login path (`partner_portal.py`'s `POST /api/partner-login` -> `partner_db.authenticate_detailed()`) verifies against `partner_db.py`'s own, separate PBKDF2-SHA256 `verify_password()`, which cannot parse a bcrypt-encoded hash and fails closed (correctly) -- re-seeded via `partner_db.password_hash()`, the same function every real customer/viewer account creation path (`partner_workspace.py`) already uses, confirmed by a direct call to both modules' `verify_password()` against the same hash. (2) A test owning both an "owner" and a "viewer" Playwright fixture that each depended on the shared `page` fixture were found, via this suite's own real first run, to alias the exact same browser tab -- the second login silently overwrote the first's session cookie, so a POST made "as the viewer" was actually authorized and audited as the owner. Fixed by giving each identity its own `browser.new_context()`. A third, cosmetic issue (`page.wait_for_function()`'s raw JS eval blocked by this site's own real CSP, `script-src` with no `unsafe-eval`) was replaced with a locator-based wait that needs no in-page eval.

**Regression**: `app/tests/test_door_access.py`/`test_relay_control.py`/`test_facial_events.py`/`test_live_view_page_face_access_ui.py`/`test_camera_access_set_camera_access.py` -- 127 passed. Full suite: 91 failed/2676 passed/24 skipped, exactly the established baseline -- zero regressions (no application code changed this pass, only the new e2e test file/script and this doc).

**Teardown**: the disposable tenant and its 16 audit rows were fully deleted after validation, and every affected table's row count verified back at the exact pre-seed baseline (`partner_users=5, appliances=3, customers=4, cameras=23, sites=4, customer_camera_permissions=5, door_access_events=0`), `PRAGMA integrity_check: ok`.

Committed and pushed to `reconcile/golden-foundation-20260911`: `e2e/tests/test_face_access.py`, `e2e/scripts/provision_face_access_test_harness.py`, `e2e/README.md`/`e2e/.env.example` updates, this doc entry.

### State to resume from (superseded by the entry below)

**Face Access staging deployment and e2e validation milestone is closed.** Staging is live on `d971110` (`deploy-portal:d971110`, `sha256:c40d3088...`), confirmed byte-identical to this commit for every Face Access source file, confirmed functionally correct end-to-end via real browser tests and a real audit trail, zero regressions. No defect was found in the shipped feature itself -- both real bugs this pass found were in the new test harness, already fixed. Physical Face Access/relay hardware, facial recognition, Talkdown, PPE, LPR plate, and People Counting line tests remain explicitly deferred until the user is on-site. AACO was explicitly not started this pass. The next development phase is WireGuard-based direct remote connectivity (preserving existing WebRTC P2P and AWS relay as fallbacks) -- not begun. Production was not touched at any point. Ryzen and Samsung were not touched at any point.

## Milestone: WireGuard direct remote connectivity -- Phase B (gateway design + appliance-agent integration + privileged-action design), source-only, nothing deployed or executed against a real interface (2026-09-17)

Continuation of the Phase A/A2 work (`debf048` audit+plan, `222e255` cloud-side schema+enrollment foundation). This pass resolved the plan's own remaining open infrastructure decision and built the next layer on top of that foundation -- still entirely source/design-level, per the user's own explicit "do not start by installing or changing WireGuard on Ryzen or staging" instruction. Full detail, including the exact architecture reasoning: `docs/wireguard-remote-connectivity-plan.md` Sec 3 and the now-filled-in Sec 20 Phase B entry.

**Gateway placement decision, resolved** (the plan's own Sec 19 open item, explicitly delegated to this pass by the user): the SAME already-built `deploy-portal` image, run as a SECOND container on the SAME existing `deploy_default` network on the SAME existing staging EC2 instance (`i-0a082abd812929bb4`) -- zero new image, zero new build pipeline, zero new compute instance. The only genuinely new infrastructure this will ever need, when actually turned on, is one new UDP port on that instance's security group plus `--cap-add=NET_ADMIN` on that one container -- `portal` gets neither. Documented template (NOT applied to real staging): `deploy/docker-compose.staging.example.yml`'s new `wireguard-gateway` service block.

**The gateway itself** (`app/wireguard_gateway/`, new package): `interface_provider.py` -- a `WireGuardInterfaceProvider` abstraction mirroring `relay_control.py`'s own `RelayProvider`/`MockRelayProvider` split exactly (`MockWireGuardInterfaceProvider` is what every test uses; `SystemWireGuardInterfaceProvider` is a real, `wg`-CLI-backed implementation, argv-shape-tested against a fake runner but never instantiated against a real interface by anything in this pass). `reconciler.py` -- a pure diff between the database's active `appliance_wireguard_peers` rows and the interface's live peer set. `proxy.py` -- a plain-HTTP forwarder with zero WireGuard-specific code (tested against a real loopback HTTP server, not a tunnel -- the whole point of the plan's own Sec 3 architecture decision is that this code can't tell the difference). `gateway_service.py` -- ties them into a real, never-started process entry point; its own module docstring records the placement reasoning in full.

**Privileged-action design** (`appliance-agent/system/privileged_watcher.py`): two new fixed-argv DISPATCH entries, `wireguard_interface_up`/`wireguard_interface_down` (`['wg-quick', 'up'|'down', '/etc/anyaicam/wireguard/wg0.conf']`) -- the hardcoded literal path points at `config_dir` (which the unprivileged agent already owns), not the OS-default `/etc/wireguard/`, specifically so the unprivileged process can write the real config content itself without ever needing write access outside its existing ownership -- the same "fixed argv reads a well-known path the unprivileged side already wrote" shape `restart_vms` already established. Every existing DISPATCH-wide safety test (no-shell-metacharacters, marker-content-never-trusted, dry-run-never-executes) automatically covers both new entries via their own `watcher.DISPATCH.items()` iteration; dedicated argv-shape tests added in `test_wireguard_privileged_actions.py`.

**Appliance-agent integration**: `config.py` gained `wireguard_identity_file`/`wireguard_conf_file` properties plus `load_wireguard_identity()`/`save_wireguard_identity()` (identical atomic-write+0600 shape to `credential.json`). New `wireguard.py` module: `generate_keypair()` (same X25519/base64 format as the cloud side, independently implemented since the two packages must stay dependency-free of each other), `render_wg_conf()`, `enroll_wireguard()`. `portal.py` gained `PortalClient.wireguard_enroll()` on the existing authenticated bearer channel -- no new appliance identity system. `setup_wizard.py`'s `_finish_enrollment()` gained a WireGuard enrollment step, gated behind `ANYAICAM_WIREGUARD_ENABLED` (unset everywhere today, matching `ANYAICAM_LIVE_P2P_ENABLED`'s own staged-rollout precedent) and always non-fatal on failure, mirroring `restart_service()`'s own established "never abort an already-successful activation over this" precedent -- proven, not just asserted, by a dedicated regression test. A re-enrollment (hardware replacement) passes `replace_existing=True`, rotating the device's WireGuard identity and revoking its prior cloud-side peer row the same way every other identity field already rotates on `coordinated_reenroll()`.

**A real ordering bug found and fixed while building the `replace_existing` cloud-side path**: the `appliance_wireguard_peers.public_key` unique index (added in `222e255`) is global, not scoped to active rows -- an initial revoke-then-enroll ordering would have hit that unique index instead of the intended "different appliance" 409 check on the narrow edge case of a client resubmitting an already-revoked key. Fixed by enrolling first (idempotent, sees accurate state) and revoking every OTHER active peer for that appliance second, explicitly excluding the row just enrolled/confirmed -- covered by 4 new tests including the exact resubmit-same-key case.

**Telemetry**: `transport-outcome` (`live_view_p2p.py`) now accepts `'wireguard'` alongside `p2p`/`relay`/`failed` -- no live caller reports it yet (Phase C has no portal wiring), accepted now so Phase C needs no route change. Heartbeat gained `wireguard_status`/`wireguard_last_handshake_at` (new `appliances` columns, COALESCE-on-omission, identical shape to `storage_state`), fed by a new best-effort local check in the agent's own `metrics.py` (`disabled`/`enrolled` only -- a live handshake check needs a privileged `wg show` call against a real interface, correctly deferred to Phase C).

**One pre-existing gap noticed but explicitly NOT touched this pass** (out of this task's scope, flagged for whoever picks up local-storage-management next): `storage_state`/`storage_free_percent`/`storage_last_cleanup_at` are read/written by `appliance_cloud.py`'s `heartbeat()` but no migration anywhere in `db_migrations.py` actually adds those three columns to `appliances` -- the same `sqlite3.OperationalError: no such column` failure class `restart_count`'s own migration comment already documents finding and fixing, appears to still exist for these three. Not reproduced/confirmed live against a real database in this pass (would require its own investigation) -- noted here only so it isn't lost.

**Tests**: 57 new tests this pass (26 in `test_wireguard_gateway.py`, 9 in `test_appliance_cloud_wireguard_status.py`, 4 `replace_existing` tests + updates in `test_wireguard_remote.py`, 1 in `test_live_view_p2p_signaling.py`; appliance-agent side: `test_wireguard_agent.py`, `test_wireguard_privileged_actions.py`, `test_finish_enrollment_wireguard.py`, plus the updated `test_rdm4_privileged_actions.py` DISPATCH-shape assertion). Cloud-side full suite: **91 failed/2734 passed/24 skipped** -- same failure/skip counts as the established baseline, zero wireguard-related failures among them, only new tests added. Appliance-agent full suite: **3 failed/536 passed/37 subtests** -- confirmed via an explicit stash/re-run A/B that the same 3 failures (all pre-existing Windows-filesystem-semantics gaps, e.g. `ENOTDIR` not raising the same way on Windows as POSIX) exist identically on the unmodified baseline; baseline was 508 passing appliance-agent tests, now 536 (+28 new, zero regressions).

**Deliberately not done this pass, per the user's own explicit scope boundary**: no real WireGuard interface was brought up anywhere; the gateway process was never started; nothing was deployed to Ryzen or staging; no portal UI; no `installer/11-install-wireguard.sh`. All Phase C work.

Committed and pushed to `reconcile/golden-foundation-20260911`.

### State to resume from (superseded by the entry below)

WireGuard Phase B (gateway design, appliance-agent integration, privileged-action design, telemetry) is complete at the source/design level, fully tested, zero regressions on either the cloud-side or appliance-agent test suites. The next step is the one the user's own directive explicitly reserves for separate authorization: bringing up a real `wg0` interface (Ryzen and/or the staging gateway container) for the first time -- everything up to that point (schema, routes, agent logic, privileged-action dispatch, gateway reconciliation/proxy logic) is real, tested code waiting for that go-ahead, not a design sketch. AACO was explicitly not started this pass. Production was not touched. Ryzen and Samsung were not touched.

## Milestone: WireGuard gateway deployed and verified live on staging (mock provider only); heartbeat migration gap re-investigated and found not real (2026-09-18)

### Heartbeat migration gap -- re-investigated, not real

The prior pass's flagged gap (`storage_state`/`storage_free_percent`/`storage_last_cleanup_at` read/written by `heartbeat()` with no migration in `db_migrations.py` adding them) was checked against the *actual* full schema-initialization pipeline, not `db_migrations.py` alone. `partner_db.py`'s own `initialize_database()` has a SEPARATE, equally idempotent ALTER-column loop (its own `for column, definition in [...]` block, lines ~144-154) that adds exactly these three columns directly -- and `ensure_database_initialized()` runs `initialize_database()` once per process, before any `connection()`-based DB access (including every heartbeat), on every container start. Confirmed empirically, not just by reading code: a fresh database gets all three columns (plus `wireguard_status`/`wireguard_last_handshake_at`/`restart_count`) after one `initialize_database()` call; an old-shaped database missing all of them, seeded with a real pre-existing row, backfills the columns correctly on the next `initialize_database()` call with the pre-existing row fully intact (`sqlite3` cross-check, not the ORM layer, to rule out any read-side masking). **No fix needed** -- the columns have been reliably added on every real process start since the 2026-09-17 local-storage-management commit; this was never actually broken the way `restart_count` once was.

### WireGuard staging deploy and live verification

Deployed `c5612b9` (env-gated `ANYAICAM_WIREGUARD_GATEWAY_PROVIDER=mock` switch + `e2e/scripts/provision_wireguard_test_harness.py`, 9 new tests, on top of Phase B's `1a038ee`/`b9566a2`) to staging via the established `verify_cutover_safety.py`/`check_container_sprawl.py`-gated recreate-in-place cutover: `CUTOVER_OK`, then `CONTAINER_SPRAWL_OK`, old container preserved stopped as `portal-d971110-pre-wireguard-gateway`. Exact commit confirmed byte-for-byte (`sha256sum` of two touched files inside the running container vs. `git show c5612b9:...`). Pre-deploy DB backup taken and checksummed (`staging-pre-wireguard-gateway-20260918T011333Z.db`). Disk headroom was tight before this deploy (93% used, 2.2GB free) -- freed via `docker image prune -a` (only images unreferenced by any container, running or stopped; every named rollback container's own image was left untouched), zero functional impact, up to 2.6GB free before building.

A second container, `wireguard-gateway-c5612b9`, was started from the same already-built image running `wireguard_gateway.gateway_service` with the new mock-provider flag -- deliberately no `--cap-add=NET_ADMIN`, no published port, confirmed in its own startup log that it will never touch a real interface.

**Real, live evidence** (full detail: `docs/wireguard-remote-connectivity-plan.md` §21):
- A disposable synthetic appliance identity (provisioned/torn down via the new harness script, never the real pilot customer) called the real `POST /api/appliance/wireguard/enroll` over real HTTPS: 200 with a real assigned `tunnel_address` (`10.70.0.2`); idempotent re-enroll; a wrong credential got a real 403; a malformed key got a real 400.
- The gateway's own log printed `WireGuard gateway reconciled: +1 -0 peers` within one real 30s poll cycle -- it read the real database over the real network and drove the mock provider to match.
- A real heartbeat carrying `wireguard_status:"enrolled"` was accepted and confirmed, by direct DB read, actually persisted on the `appliances` row.
- **Fallback proof**: 23 real Playwright browser tests (`test_live_view_p2p.py` x4, `test_live_view.py` x10, `test_camera_status.py`/`test_dashboard.py` x9) ran against this exact deployment -- gateway container running alongside -- and all passed. WebRTC P2P and the existing live-view/transport-racing paths are provably unaffected by the gateway's mere presence.
- `PRAGMA integrity_check: ok`; every row count (`partners`/`customers`/`sites`/`appliances`/`partner_users`/`cameras`) identical before and after, both before and after the disposable harness's own teardown; `appliance_wireguard_peers` (created fresh by this deploy's own migration) ended at exactly 0 rows.
- Resource headroom essentially unchanged post-deploy (gateway container ~10MB RAM idle; 2.3GB memory / 2.4GB disk available) -- no repeat of this project's own documented OOM/disk-exhaustion incidents.

**Full regression**: 91 failed/2747 passed/24 skipped -- established baseline, zero new regressions. Commit `c5612b9`, pushed to `reconcile/golden-foundation-20260911`.

### Ryzen -- read-only inspection only, confirms the real next step is smaller than "bring up wg0"

`wireguard-tools` is confirmed **not installed** on the real Ryzen appliance (`which wg wg-quick` empty). The currently-running `anyaicam-agent.service` and `privileged_watcher.py` predate this entire WireGuard effort -- no `wireguard.py`, no `ANYAICAM_WIREGUARD_ENABLED` gate, no `wireguard_interface_up`/`down` DISPATCH entries exist on the real box yet. `wg-quick up /etc/anyaicam/wireguard/wg0.conf` (the DISPATCH entry itself) cannot do anything today -- the binary doesn't exist, the config file doesn't exist (nothing has enrolled), and no real (non-mock) gateway with a reachable UDP endpoint exists anywhere yet either (that is itself a separate, its-own-authorization infrastructure step, not part of this pass's scope). The genuine first privileged/sudo step in the real chain is installing the package itself -- see the coordinator's own report for the exact command and explanation. This was investigated read-only only, per this project's own standing Ryzen access model; nothing was installed, configured, or changed on Ryzen.

### State to resume from (superseded by the entry below)

WireGuard gateway + enrollment + reconciliation + telemetry are now proven live end to end on staging, mock-provider-only, zero real interface anywhere, zero regressions, fallback to WebRTC P2P/AWS relay empirically proven unaffected. The heartbeat migration-gap concern from the prior pass is resolved (was never actually broken). Remaining before a real tunnel exists anywhere: (1) install `wireguard-tools` on Ryzen (sudo, the pass's own stopping point); (2) redeploy the updated appliance-agent code to Ryzen via the established release-installer pipeline; (3) set `ANYAICAM_WIREGUARD_ENABLED=true`; (4) stand up a real (non-mock) cloud gateway with a real reachable UDP endpoint -- its own separate authorization; (5) only then a real `wg-quick up`. AACO was not started. Production was not touched. Ryzen was inspected read-only only, nothing changed.

## WireGuard -- step (1) done by the operator; step (2)'s artifact built and staged; the real gateway/DISPATCH mechanism confirmed already fully implemented; the next real privileged step identified (2026-09-17, later)

**Step (1) confirmed complete**: the operator ran `sudo apt-get install -y wireguard-tools` on the real Ryzen appliance themselves and confirmed success (`wireguard-tools 1.0.20210914-1ubuntu4`, `wg-quick.target` present but not started -- no interface exists yet, exactly as expected from a package install alone).

**Source-side audit this pass, no code changes needed**: `SystemWireGuardInterfaceProvider` (`app/wireguard_gateway/interface_provider.py`) and the `wireguard_interface_up`/`down` DISPATCH entries (`appliance-agent/system/privileged_watcher.py`) were already fully implemented and already covered by real tests in the prior pass (62/62 WireGuard-suite tests re-run clean this pass, `PYTHONPATH=app pytest app/tests/test_wireguard_gateway.py app/tests/test_wireguard_remote.py app/tests/test_appliance_cloud_wireguard_status.py`) -- there was nothing left to build for either. Keypair generation is confirmed automatic: `setup_wizard.py`'s `_finish_enrollment()` calls `enroll_wireguard()` (which generates a fresh X25519 keypair locally only if none already exists, never overwriting one on a routine reconnect) whenever `ANYAICAM_WIREGUARD_ENABLED=true`, then auto-queues `wireguard_interface_up` on success -- no separate manual trigger is needed once the flag is on.

**Ryzen agent version, confirmed via the one channel available (no sudo, no direct file read)**: `/version` on the real running VMS container reports `build_id d838d8c...` -- a commit from well before every WireGuard commit in this branch's history (and before Face Access too). Direct inspection of `/opt/anyaicam-agent/` itself was correctly blocked by file permissions (root-owned, the operator's own non-privileged SSH key has no read access) -- expected, not a problem, and unnecessary anyway since this project's release pipeline has always bundled VMS + agent into one artifact/install, so the agent is certainly on the same old code with none of this feature's changes.

**Release artifact for the current commit, built and staged**: `installer/build_release_installer.py --vms-commit 8d5a075c3ccf9ca5dd77ce135ad5b5f57e905610 --vms-repo .` -- `anyaicam-appliance-installer-1.1.0-vms-8d5a075c3ccf.tar.gz`, SHA-256 `0d910a589cfbefad44d0b5211f69c923c395bedf89882533dae9f60ac66aafea`, all built-in checks passed (13 shell scripts, LF-only, executable bits correct, no secrets). `scp`'d to Ryzen's own home directory (no sudo needed for a file copy) and checksum-reverified identical there. Also extracted (non-privileged `tar xzf`) into `~/anyaicam-release-8d5a075c/` on Ryzen, so the one remaining step really is just the privileged install/validate call, not any unfinished staging.

**Staging security group, read-only audit (`sg-0a7cebe64d0e3d752`, instance `i-0a082abd812929bb4`)**: only inbound TCP 80/443 from `0.0.0.0/0` exist today -- no UDP port of any kind is open. This confirms the plan's own Sec 16 expectation: standing up a real (non-mock) cloud gateway with a reachable endpoint needs one new inbound UDP security-group rule (the standard WireGuard port, 51820, is the obvious choice, source `0.0.0.0/0` since Ryzen's own public IP is NAT'd/dynamic and the tunnel's real authentication is WireGuard's own cryptographic handshake, not source-IP filtering). Not created this pass -- treated as needing its own explicit go-ahead, same as every other real network/interface change, even though most staging work is otherwise autonomous.
...
## Milestone: `anyaicam-setup --wireguard-enroll` -- the standalone trigger for an already-activated appliance (2026-09-17)

Built the missing piece the prior pass identified but deliberately didn't build unilaterally: a small, additive CLI entry point that lets an appliance activated before `ANYAICAM_WIREGUARD_ENABLED` existed (every real appliance in the field, including Ryzen) reach `wireguard.py`'s already-built `enroll_wireguard()` without going through `_finish_enrollment()` (which only ever runs from the one-time `interactive_main()`/`claim_main()` activation flows, never from `service.py`'s long-running daemon).

**`setup_wizard.wireguard_enroll_main()`** (new), dispatched via `anyaicam-setup --wireguard-enroll` (`main()`'s existing `sys.argv` check, same pattern as `--claim`). Deliberately narrow: reads the appliance's existing `credential.json` via `load_credential()` (fails loud with a clear message if the appliance was never activated -- never invents or requests one), builds a `PortalClient` from it, and calls `enroll_wireguard(config, client, replace_existing=False)` directly -- the exact same function `_finish_enrollment()`'s own gated hook already calls, not a reimplementation. **Never imports or calls `first_enroll()`/`coordinated_reenroll()`/anything in `reenrollment.py`** -- this command's entire contract is "reuse the existing identity, touch nothing about it": `agent.json`, `credential.json`, and the VMS's own `appliance_identity.json` are never read, staged, or written anywhere in this function, verified by test (`test_first_run_preserves_existing_identity_files_byte_for_byte`, byte-for-byte before/after) and by the fact that `first_enroll`/`coordinated_reenroll` are asserted never-called.

**Idempotency**: `enroll_wireguard()` itself already reuses an existing local keypair and the cloud side (`enroll_peer()`, prior pass) already returns the existing row for a resubmitted public key -- both untouched here. This command adds one more layer on top, purely for the *operational* question of whether the real interface needs re-queuing: it reads the local WireGuard identity before calling `enroll_wireguard()` and compares it to what comes back afterward. Unchanged (the common case -- a routine re-run or a service restart) means a clean no-op, no `wireguard_interface_up` re-queue. Changed (first-ever run, or the cloud side reconciled to a new tunnel/gateway assignment, e.g. after a revoke) means the interface-up action is queued, exactly once per real change -- never once per invocation.

**Tests** (`test_wireguard_enroll_cli.py`, 12 new): not-yet-activated fails loud without calling enrollment; first run on an already-activated appliance calls `enroll_wireguard()`, queues interface-up, and leaves `credential.json`/`agent.json` byte-identical; running it twice in a row calls `enroll_wireguard()` both times (idempotency is that function's job, not skipped here) but only queues the interface-up action once; an already-WireGuard-enrolled appliance with identical config is a clean no-op; a genuine cloud-side change (simulated revoke-and-reassign) correctly re-queues; a `PortalError` mid-enrollment raises `SystemExit` with nothing touched locally and no partial WireGuard identity written, and a subsequent successful retry recovers cleanly; a failed interface-up queue is a warning, not a crash; the `--wireguard-enroll` CLI flag dispatches correctly.

**Regression**: appliance-agent suite, real A/B via `git stash` (only `setup_wizard.py`'s modified, the new test file is untracked so stash doesn't touch it): before, 15 failed (12 new tests failing without the source change + the same 3 pre-existing Windows-filesystem-semantics failures)/536 passed; after, 3 failed (the identical 3 pre-existing)/548 passed. Zero regressions, 12 new tests, all green. No cloud-side (`app/`) file touched this pass -- `app/tests/` unaffected by construction, not re-run.

### The one real thing still worth flagging, not fixed this pass

`wg-quick up` on an interface that's already up errors rather than no-ops. This command's own reconcile logic avoids *redundantly* re-queuing `wireguard_interface_up` on an unchanged config, but does not (and structurally cannot, from the unprivileged agent side) know whether a *previously queued* action was ever actually consumed and applied by the root-owned `privileged_watcher.py`. In the realistic sequence this project has followed for every other privileged action so far (queue once, a human confirms and the watcher applies it, done), this is a non-issue. It would only matter if this command were run again after a real interface change was queued but never applied, and only manifests as one `wg-quick up` invocation harmlessly failing on an idempotent no-op case -- not a corruption or duplication risk, and not worth adding speculative interface-state-probing logic to an unprivileged process for a scenario that hasn't happened yet.

### Is this ready to run on Ryzen?

`anyaicam-setup --wireguard-enroll` runs as the existing unprivileged `anyaicam` system user -- the same user every other `anyaicam-setup` invocation already runs as (`appliance-agent/scripts/install.sh`'s own documented `sudo -u anyaicam ...anyaicam-setup`). The `sudo` in that invocation is only to assume that already-unprivileged account's identity from an operator's own login shell, not to gain root -- this command never touches `/etc/wireguard/`, systemd, Docker, or anything requiring real privilege; it only reads/writes files under `/etc/anyaicam` (already owned by `anyaicam`) and makes one authenticated HTTPS call. It queues `wireguard_interface_up` for the separate, already-existing root-owned watcher to pick up -- it does not and cannot bring up the interface itself. Still being surfaced explicitly, not run automatically, because it is new code executing for the first time against the real appliance's real identity.

**Proposed command (not yet run):**
```
sudo -u anyaicam anyaicam-setup --wireguard-enroll
```
**What it changes**: submits this Ryzen's real WireGuard public key (generated fresh, on-device, only on this run) to the cloud gateway's enrollment endpoint, writes `/etc/anyaicam/wireguard_identity.json` and `/etc/anyaicam/wireguard/wg0.conf` (both new files, 0600, owned by `anyaicam`), and queues a `wireguard_interface_up` marker for the privileged watcher. It does **not** create, start, or configure any real network interface, route, or firewall rule by itself -- no `wg0` exists after this command alone finishes. It does not touch cameras, recordings, `credential.json`, `agent.json`, customer/site assignment, or any existing systemd unit.

**Expected result**: prints a real tunnel address (something in `10.70.0.0/16`) and the configured gateway endpoint, then "WireGuard interface bring-up has been queued for the privileged watcher to apply." A `docker exec anyaicam-vms` DB read (or the cloud-side `appliance_wireguard_peers` table, same way the prior pass independently confirmed the schema landed) will show one new active peer row for this appliance. `credential.json`/`agent.json` should be byte-identical to before (`sha256sum` both before and after, same discipline as every prior post-deploy check).

Still needed after that, each its own separately-gated step, unchanged from before: the staging UDP 51820 security-group rule (proposed, not created) and standing up the real (non-mock) cloud gateway -- **this command will currently fail with a clear 503 ("WireGuard gateway is not configured on this environment")** if run before either of those exists, since `GATEWAY_PUBLIC_KEY`/`GATEWAY_ENDPOINT` are unset anywhere real yet (`app/wireguard_remote.py`'s own fail-closed default). Worth sequencing the gateway stand-up before this Ryzen command for that reason, even though nothing prevents running this command first and simply seeing the expected 503.

### State to resume from

WireGuard's enrollment-trigger gap is closed, tested, and pushed. The real remaining sequence, in order: (a) the real (non-mock) cloud gateway on staging (needs the UDP 51820 security-group rule first, both still proposed, not created -- see the prior milestone's own investigation); (b) only then does `anyaicam-setup --wireguard-enroll` above have a live gateway to actually enroll against instead of a 503; (c) the real Ryzen privileged command itself, proposed above, not yet run. AACO not started. Production not touched.

## Milestone: Hybrid transfer-cost reduction audit -- real caching fix implemented, CloudFront/storage-tiering evaluated and correctly not built, margin model refined (2026-09-18)

Follow-up to the same day's earlier Hybrid dedup audit (`e013fe3`), which found data-transfer-out dominates cost and $9.99/camera/month has negative margin at real busy-camera usage. This pass asked whether that cost is reducible through architecture, without touching WireGuard/MediaMTX (a separate, frozen workstream) or the PPE/LPR/facial-recognition media gap (still reserved for its own pass). Full detail, real numbers, and every assumption's own label: `docs/hybrid-transfer-cost-reduction-audit.md`.

**Root cause of the transfer-out dominance, traced to real code**: event/recording media is served through a second S3 bucket (`anyaicam-recordings-2026`) this codebase's already-built, already-proven CloudFront-signing mechanism (`app/live_cdn_signing.py`) has never been wired to -- it only fronts the separate live-relay bucket (`anyaicam2026`). Every event clip/thumbnail fetch is a direct S3 GetObject via a presigned URL, and -- confirmed by direct grep -- neither the object's own S3 metadata nor the redirect routes handing out the URL ever set a `Cache-Control` header, and the URL itself was re-signed fresh on every single call. A real, traced consequence: the customer Dashboard's "recent events" widget rebuilds every thumbnail `<img>` on a 15s poll (`updateRecentEvents()`), and the mobile view polls as fast as 4s while an event is pending (`MOBILE_EVENT_POLL_INTERVAL_MS`) -- both fully re-fetching unchanged thumbnails from S3 every cycle, uncapped, for as long as a page stayed open.

**Fixed** (`app/main.py`, `app/event_media_uploader.py`, `app/recording_uploader.py`): a presigned-URL reuse cache (300s window, safely inside the object's real 900s validity), a `private, max-age=<n>` `Cache-Control` header on both thumbnail redirect routes sized to that window, and a long-lived `immutable` `CacheControl` on all newly-uploaded clip/thumbnail/recording objects. Measured effect on the specific waste this closes: a wall-mounted/always-open dashboard (a normal real usage pattern for this product) went from an uncapped ~$2.33/camera/month in thumbnail-redraw waste down to ~$0.12, a 20x reduction. Explicitly **not** claimed to move the dominant driver -- clips are ~41x larger than thumbnails and fetched on click, not on a poll timer, so this fix closes a real, previously-unbounded liability on the minority cost driver, not the majority one.

**Investigated and correctly not built, with real reasons**: extending CloudFront to the recordings bucket is a real infra change (out of this pass's scope) and, worked out precisely, a smaller lever than it first looks -- CloudFront's own egress rate is only ~6% cheaper than S3 direct here, and same-region S3-to-CDN transfer is already free, so its real incremental value is narrower (cross-device/cross-session repeat views) than "caching saves money" intuition suggests; still recommended for architectural consistency with the live-video path, just not oversold. S3 Intelligent-Tiering was checked directly against Hybrid's own 30-day retention and found to add a real monitoring fee for zero realized saving, since a 30-day-retained object is deleted right around the same 30-day point it would first become tier-eligible -- correct conclusion: stay on S3 Standard. The single biggest real lever for the dominant driver -- serving a recent event clip directly from the appliance to a remote viewer with a direct/LAN/WireGuard path, avoiding AWS transfer entirely, mirroring the existing WebRTC-P2P-first/AWS-relay-fallback precedent for live video -- was identified but deliberately not designed further, since doing so would mean touching the explicitly frozen WireGuard workstream.

**Recalculated margin model**, framed directly around the user's own requested 5/10/25/50/100%-viewed brackets (a more realistic shape than the prior pass's 1x/2x-per-event multiplier, though the two agree almost exactly at the 100%-viewed/"1x" overlap point -- $8.78 vs $8.77 at 1000 events/day, confirming this is a refinement, not a contradiction): **Normal (200/day, 10% viewed) and Busy (500/day, 25% viewed) now show real, comfortable margin** ($0.55 and $1.87 against $9.99). **Very Busy (1000/day, 50% viewed, $5.42) still has margin but it's narrowing**, and sits below the real fleet's own already-observed busiest cameras. **The advertised 9-hour/day ceiling, if approached, still has negative-to-thin margin** ($16.88-$27.36 depending on view rate) -- this pass's fixes don't change that conclusion, because clip transfer at the ceiling dwarfs everything touched here. Storage and request costs remain single-digit-percent of total cost at every scenario -- confirmed, again, that the entire margin question is the real-world view rate at real-world volume, not storage.

**Regression**: targeted suite 86 passed/0 failed (includes 2 pre-existing tests correctly updated to patch the real post-refactor call target, plus 9 new tests). Full suite: 91 failed/2758 passed/24 skipped -- the established baseline failure count held exactly.

**No production system, AWS infrastructure, CloudFront/S3 configuration, or WireGuard/MediaMTX/installer file was touched.**

### State to resume from

Both Hybrid pricing-audit passes (`e013fe3`, this one) are complete and pushed. Real, implementable engineering was applied where it was real (thumbnail-redraw waste, now bounded) and correctly withheld where the numbers didn't support it (CloudFront's true incremental value, S3 tiering). The margin picture is now nuanced rather than uniformly bad: Normal/Busy usage has real margin, Very Busy is tight, the advertised ceiling does not. The single highest-value remaining lever (local/direct/WireGuard-preferred event playback, avoiding AWS transfer entirely for a remote viewer with a direct path) is explicitly identified but deferred until the frozen WireGuard workstream resumes with its own separate approval. Real per-clip view-rate telemetry remains the standing recommendation to replace the 5-100% bracket with a real number before locking in a price. $9.99/camera/month itself was not changed -- that decision remains the user's. AACO not started. WireGuard remains frozen exactly at `c332aa9`, untouched by this pass. Production not touched.

**`a11416a` is the preserved Hybrid transfer-cost optimization checkpoint -- do not re-run or redo this pass's work in a future session.** Hybrid pricing/cost work is on hold here, by explicit user instruction, with exactly two open follow-ups, neither to be started before WireGuard resumes and is live-measured:

1. **Revisit the 9-hour/day advertised ceiling** once WireGuard/direct playback is complete and measured -- the ceiling's own worst-case cost ($16.88-$27.36/camera/month) was explicitly not resolved by this pass and needs the real transfer-avoidance number from item 2 below before it can be re-evaluated honestly.
...
## Milestone: real bug found and fixed -- re-running the enrollment script alone would NOT have refreshed the stale endpoint (2026-09-18)

The prior entry's proposed fix ("just re-run `ryzen_wireguard_enroll_only.py`") turned out to be necessary but not sufficient. The user re-ran it after the portal recreate and it **still returned the stale placeholder endpoint** -- their own diagnosis was exactly right: "portal env is correct, but enrollment output is still stale, so something else is overriding it."

**Real root cause, confirmed against the live staging DB, not guessed**: `wireguard_remote.py`'s `enroll_peer()` persists `GATEWAY_PUBLIC_KEY`/`GATEWAY_ENDPOINT` (module-level constants read from env at import) into the `appliance_wireguard_peers` row at INSERT time -- but its idempotent-reconnect path (same `public_key`, no rotation -- exactly what a re-run of the enrollment-only script produces, since it doesn't regenerate Ryzen's keypair) returned the **existing row verbatim**, never re-checking it against the current live env values. Confirmed live: Ryzen's peer row (`id=9cd86a0ab4794779`, `created_at=2026-09-18T12:08:04`) still held `gateway_endpoint=staging-wireguard-gateway.internal.anyaicam.test:51820` and the old `gateway_public_key=A1ZJFEotRgb3S5kp5nO/...` -- both frozen at whatever was live at the row's original INSERT, well after the portal's own env was corrected. The portal env, the DB schema, and the queuing/dispatch mechanism (proven sound in the prior entry) were all fine; only this one idempotent-return path was stale.

**Fixed** (`app/wireguard_remote.py`, commit `69a6e80`): `enroll_peer()` now refreshes `gateway_public_key`/`gateway_endpoint` on every idempotent re-enroll when they differ from the current live values -- writing only those two columns; `tunnel_address`, `public_key`, and the row's `id` are never touched, so Ryzen's own identity and address assignment are completely unaffected by this fix. No key rotation, no re-enrollment from scratch, no data migration needed -- once this fix is deployed and the enrollment script is re-run, the existing row will self-correct via this same idempotent path rather than needing a manual one-time DB correction.

**New regression test** (`test_reenrolling_after_a_gateway_config_change_returns_the_current_endpoint`) reproduces the exact real sequence: enroll, change the module-level gateway config (mirroring a redeploy), re-enroll with the same `public_key`, assert the second response reflects the new values and the peer count stays at 1 -- exactly the shape of test that would have caught this before it ever reached a real appliance.

**Regression**: targeted suite 32/32 passed (31 existing + 1 new). Full suite: 91 failed/2760 passed/24 skipped -- established baseline held exactly, +1 for the new passing test.

**This pass was code-only, per its own explicit scope boundary** -- no redeploy to staging, no re-run of the enrollment script, no fresh `wireguard_interface_up` queue action. Ryzen's identity/peer assignment, the gateway's own keypair, and every other already-validated piece (staging portal, staging gateway, 5 cameras, Ryzen VMS) were untouched.

### State to resume from

The real stale-endpoint bug is found, fixed, tested, and pushed (`69a6e80`) -- code-only, nothing deployed or re-run yet. Next steps, in order, each still its own separately gated action where applicable: (1) deploy this fix to the staging portal (an autonomous staging recreate, same pattern as the portal-env-correction recreate); (2) re-run `ops/ryzen_wireguard_enroll_only.py --yes` on Ryzen (non-privileged, no network/interface change -- should now return the corrected endpoint, self-healing the stale DB row via this fix's own idempotent-refresh path) and independently verify the returned endpoint is `34.194.19.113:51820` before proceeding; (3) only then a fresh gated `wireguard_interface_up` queue action, followed by the full tunnel validation (handshake, 5-camera testing, fallback proof, playback-path measurement) that was blocked last time. Production not touched. Hybrid pricing/source untouched, still checkpointed at `a11416a`. AACO not started.

## Milestone: real Ryzen Tailscale/SSH outage -- root-caused to a full-tunnel `AllowedIPs = 0.0.0.0/0` in the appliance's own rendered `wg0.conf`, fixed in source, not yet deployed or re-applied to Ryzen (2026-09-18)

After the stale-endpoint fix above was deployed to staging and the enrollment script re-run, a fresh `wireguard_interface_up` was queued and (per the operator) completed. The real Ryzen appliance then went unreachable over Tailscale/SSH -- `tailscale status` showed it `offline, last seen`, growing longer across repeated checks, `tx>0 rx 0` (traffic sent, nothing back); SSH timed out on both its Tailscale and LAN addresses. The appliance's own HTTPS cloud heartbeat kept working the whole time -- a live asymmetry that turned out to be the actual diagnostic clue.

**Root cause, confirmed directly in source** (`appliance-agent/anyaicam_agent/wireguard.py`'s `render_wg_conf()`, the exact function this same file's earlier docstring already flagged as unaudited for this specific risk): the `[Peer]` block hardcoded `AllowedIPs = 0.0.0.0/0`. WireGuard's own standard, well-documented behavior for that value is to install a full default-route override into the host's main routing table the instant `wg-quick up` runs -- unconditionally, regardless of whether a handshake ever succeeds. The queued interface-up on Ryzen never completed a real handshake (confirmed on the gateway side: the registered peer had no `latest handshake`/`transfer` line at all), so every packet the OS routed into the tunnel simply vanished -- including Ryzen's own SSH and Tailscale traffic, both of which use the host's normal routing table. The containerized VMS app's HTTPS heartbeat kept working only because Docker's own NAT path never touches the host routing table at all; that's the asymmetry that made this diagnosable without device access. The module's own pre-existing comment claimed this AllowedIPs value "does NOT affect the appliance's real LAN-facing interface or its normal internet route at all" -- true for a narrow value, false for `0.0.0.0/0`; the comment was simply wrong.

**Recovery**: the operator physically rebooted Ryzen, which cleared the hijacked routing table (a fresh boot re-establishes the normal default route before anything -- including the not-persistent-across-reboot `wg0` -- comes back up). Tailscale ping and SSH are confirmed restored. `wg0` was never brought back up as part of recovery and must stay down until explicitly re-approved.

**Fixed in source** (`app/wireguard_remote.py`, `appliance-agent/anyaicam_agent/wireguard.py`; commit pending push this pass): the cloud enrollment response now includes a new `gateway_tunnel_address` field (`app/wireguard_remote.py`'s new `GATEWAY_TUNNEL_ADDRESS`, sourced from the same `ANYAICAM_WIREGUARD_GATEWAY_ADDRESS` env var `app/wireguard_gateway/gateway_config.py` already uses for the gateway's own interface address, default `10.70.0.1` -- one real value, not a second guess at it; read fresh on every response, never persisted, so it can never go stale the way `gateway_public_key`/`gateway_endpoint` did in the prior incident). `render_wg_conf()` now takes `gateway_tunnel_address` as a required (no-default) parameter and writes `AllowedIPs = {gateway_tunnel_address}/32` -- exactly one host, never a range. This is correct for the design, not just "narrower": the plan doc's own hub-and-spoke topology means an appliance only ever needs to reach the gateway over this tunnel, never another appliance directly, so the gateway's own `/32` is both necessary and sufficient. `enroll_wireguard()` threads the new field from the portal response straight into the identity dict and the rendered config -- a missing field is a loud `KeyError`, not a silent fallback to something dangerous, matching this codebase's own fail-closed convention. Checked and confirmed no second instance of this bug class exists elsewhere in the WireGuard code: the gateway's own peer `AllowedIPs` (`app/wireguard_gateway/reconciler.py`'s `desired_peers_from_rows()`) has always been structurally derived as `f"{tunnel_address}/32"` per-appliance, never a broader literal.

**New regression tests**: `app/tests/test_wireguard_remote.py` gained a structural single-host proof (`ipaddress.ip_network(..., strict=True).num_addresses == 1`) and a live-vs-stale-env check for the new field. `appliance-agent/tests/test_wireguard_agent.py` gained a full `AllowedIpsNeverInstallsAFullTunnelRouteTests` class (parametrized structural proof across multiple addresses, an explicit `!= '0.0.0.0/0'` check, and a test proving the parameter has no default at all -- a caller cannot omit it and silently get something broad) plus an end-to-end test proving the portal's response value actually reaches the written `wg0.conf` file, not just that `render_wg_conf()` is correct in isolation.

**Regression**: targeted WireGuard suites clean (44/44 appliance-agent, 74/74 cloud-side). Full appliance-agent suite: 552 passed/3 failed (the same 3 pre-existing Windows-filesystem-semantics failures, +4 for the new tests -- zero regressions). Full cloud-side suite: 2761 passed/92 failed/24 skipped -- one more failure than the most recently recorded baseline (91); investigated directly rather than assumed away: every failing test is in a file this pass never touched (`test_recording_uploader_credential_hardening.py`, `test_recording_uploader_newest_first.py`, `test_retention_worker_responsiveness.py`, `test_settings_navigation.py`, and the already-documented-elsewhere-as-flaky `test_talk_audio_relay.py::test_no_appliance_channel_uses_local_isapi_fallback`), and re-running the two recording-uploader files in isolation produced a *different, larger* failure count (19 vs. 11 in the full run) -- proof these are order/interaction-dependent pre-existing test fragility, not a regression from this change.

**Not done this pass, per explicit scope**: nothing deployed to staging, Ryzen not touched at all (unreachable for most of this pass; SSH access was restored only near the end, and the explicit instruction was to preserve that working connection, not risk it), `wg0` not brought up anywhere, no security-group change.

### State to resume from (superseded by the entry below)

The full-tunnel-route root cause is fixed in source, tested, and ready to push. Ryzen is physically recovered (reboot cleared the hijacked routing table) with `wg0` correctly still down. **Deploying this fix to staging is the correct next single step** -- not re-running the enrollment script on Ryzen yet, since the *live staging endpoint* doesn't have this fix applied yet either: re-enrolling right now would call the still-unfixed `/api/appliance/wireguard/enroll`, which doesn't return a `gateway_tunnel_address` field at all, so the now-required parameter would raise immediately (a loud, safe failure, but not a productive one) rather than silently reverting to anything dangerous. Once deployed, the sequence resumes in order: (1) staging deploy of this fix [next], (2) re-run `ops/ryzen_wireguard_enroll_only.py --yes` on Ryzen -- purely re-renders `wg0.conf` with the corrected `/32` AllowedIPs, writes no interface, brings nothing up, and is safe to run now that SSH access is confirmed restored, (3) independently verify the rendered config's `AllowedIPs` line, (4) only then a fresh, separately gated `wireguard_interface_up` queue action. Production not touched. Hybrid pricing/source untouched, still checkpointed at `a11416a`. AACO not started.

## Milestone: `AllowedIPs` fix deployed to staging (`cc0d266`), verified live -- Ryzen not touched (2026-09-18)

Deployed the full-tunnel-route fix to `anyaicam-staging` via the same established recreate procedure used for every prior deploy this session: built `deploy-portal:cc0d266` on-host from the existing git clone (`/opt/anyaicam-staging-build/anyaicam-d971110`, avoiding the Windows `core.autocrlf` hazard entirely), stopped and renamed the running container to `portal-c5612b9-pre-allowedips-fix` (preserved, not deleted -- rollback intact), disconnected its network aliases, and started the new `portal-c5612b9` with the identical `deploy_default` network, `portal`/`vms` aliases, volume mounts, env-file, command, and restart policy.

**Verified, all real evidence**: internal health `{"status":"ok",...}`; real public `https://portal-staging.anyaicam.com/health` -> `200` (the host-loopback `curl -k https://127.0.0.1:443` check returned a connection failure, but this was a TLS/SNI artifact of that specific test method against a raw IP target, not a real outage -- the actual public domain check is the one that matters and it's clean; no Caddy/NAT disruption symptom, no restart needed this time). DB row counts (`partner_users: 5, appliances: 3, customers: 4, cameras: 23, appliance_wireguard_peers: 1`) and `integrity_check: ok` identical before and after. Deployed code confirmed byte-for-byte: `sha256sum` of `/app/wireguard_remote.py` inside the running container matches `git show cc0d266:app/wireguard_remote.py` exactly (`8ee2e3a3...`). Functionally confirmed live: the response-construction code at `wireguard_remote.py:311` includes `"gateway_tunnel_address": GATEWAY_TUNNEL_ADDRESS` (line 90, sourced from `ANYAICAM_WIREGUARD_GATEWAY_ADDRESS`, defaulting to `10.70.0.1` -- matching the real gateway's actual tunnel address, no env override needed) -- since the running file is proven byte-identical to this source, this is live in the deployed container, not just proven in the repo.

**Ryzen was not touched in any way this pass** -- no SSH attempt, no enrollment script, no privileged action queued, `wg0` remains down exactly as the user left it after their physical reboot.

### State to resume from

The fix is live on staging. The sequence continues in order, each its own separately gated step: (1) [done] staging deploy; (2) re-run `ops/ryzen_wireguard_enroll_only.py --yes` on Ryzen to re-render `wg0.conf` with the corrected `/32` `AllowedIPs` (writes config only, brings nothing up, safe now that SSH is confirmed restored) -- needs the user's own go-ahead; (3) independently verify the rendered config's `AllowedIPs` line before proceeding further; (4) only then a fresh, separately gated `wireguard_interface_up` queue action. Production not touched. Hybrid pricing/source untouched, still checkpointed at `a11416a`. AACO not started.

## Milestone: overnight release-readiness pass -- analytics, admin access, Stripe delayed payments, installer rollback; golden `8e1f166` (2026-09-25)

Golden `reconcile/golden-foundation-20260911` moved `2d86193` -> `8e1f166` through four separate non-force merges, each of a branch pushed to origin. Nothing was installed on, staged on, or changed on the Ryzen; no production Stripe/AWS/DNS change was made.

- **Analytics** (`2cc9a69`, `feature/analytics-release-pass-20260925`): customer-placeable People Counting line (new `people_counting` rule type in the tenant-safe rule editor; the worker prefers it and falls back to the legacy local `analytics_rules.json`, so Ryzen camera 1's existing line keeps counting); fixed inbound-/outbound-only counting lines never counting anything; intrusion/line-crossing evaluator limited to its own rule types and gated on each camera's Smart Motion entitlement; the installer now adds `CUSTOMER_ANALYTICS_RULES_ENABLED`, `PEOPLE_COUNTING_ENABLED`, `ANYAICAM_LPR_ENABLED`=true when missing (never overwriting an explicit value); Face Access ArcFace engine packaged (`onnxruntime==1.19.2`, YuNet/SFace/ArcFace baked into `/opt/anyaicam-aac-models`, SHA-256 pinned) with `ANYAICAM_FACE_ENGINE` still defaulting to `haar`; facial sync warns when no enrollment matches the active engine.
- **Admin access** (`73aa2f7`, `fix/admin-access-lockout-20260925`): the last live global administrator grant can no longer be revoked (break-glass cannot recover a revoked grant); the appliance's local user API can no longer disable/demote/delete the last user manager; a voluntary password change now requires the current password (forced first-password changes excepted).
- **Stripe** (`0cee627`, `fix/stripe-delayed-payment-20260925`): a Checkout Session completed with `payment_status=unpaid` (delayed payment methods) grants no camera slots/analytics, creates no hardware order or pending link; `checkout.session.async_payment_succeeded` grants. **Needs, in the Stripe Dashboard: subscribe the webhook endpoint to `checkout.session.async_payment_succeeded` (and `_failed`).**
- **Installer** (`22cf9ca`, `fix/installer-rollback-point-20260925`): every repair/upgrade first tags the previous image `anyaicam-vms:rollback-<commit>`, archives the previous code, takes an online integrity-checked database backup and writes `/var/lib/anyaicam/rollback/latest.env`; `installer/rollback.sh` restores from it (database only with `--restore-database`, keeping the replaced one); `validate.sh` gained a read-only database integrity check and a rollback-point check.

**Tests**: full app suite on the tested tip, in an isolated worktree: 3856 passed / 87 failed -- the 87 are exactly golden `2d86193`'s baseline set (0 new, 0 fixed). Installer suite 32 passed (Windows) and 18/18 rollback + product-mode on Linux (WSL, real rsync).

**Parked, not merged**: `feature/camera-capability-model-20260925` (`c7ace41`) -- camera capability model schema 2, deprioritized in favor of the analytics pass.

**Installer built, not staged**: `anyaicam-appliance-installer-1.1.0-vms-8e1f166a6f28.tar.gz`, SHA-256 `b139b3c1028c67e3b6eff73ffcc15ab3abbffcce68e4c93da63cffa2b8c2c6d3` (Dell scratch build; rebuild from `8e1f166` if it is gone). The Ryzen still runs `34f144d`; its manual rollback points (`rollback-34f144d`, `rollback-88ad9e7`) are untouched.

### State to resume from

Next: stage the `8e1f166` installer on the Ryzen and install with `--repair` (the first install that creates its own rollback point), then the physical validation list: a counting line placed from the customer portal plus a walk-through, an intrusion zone entry, a real plate for LPR, PPE with real gear, and -- after switching `ANYAICAM_FACE_ENGINE=arcface` on staging and the Ryzen and re-adding reference photos -- Face Access at door distance plus a second-person check.

## Milestone: Analytics result snapshots before playback deployed to staging (`2ccbe34`), validated on real data -- Ryzen not touched (2026-09-26)

**Reported**: Analytics result cards were black before playback; clicking played the clip correctly, but nothing identified a result before it was opened.

**Root cause (real staging data, read-only probe inside `portal-eb3002e`, last 14 days)**: staging was still on `eb3002e`; the fix (`b5611cc`) had not been deployed. The stored snapshots themselves were fine -- every Smart Motion event with a clip has one, and sampled snapshots on all 5 cameras returned `200 image/jpeg` in ~70-300 ms with normal (non-black) content, so read-role IAM, snapshot keys and presigning were not the problem. The deployed cards had two faults: (1) each card downloaded the full stored frame (150-480 KB at 1280x720, up to 860 KB at 3840x2160) behind a near-black `#0b1018` background, so cards sat black while loading, and a failed load removed the image (`onerror="this.remove()"`) leaving plain black; (2) results with no media (about half of Smart Motion) rendered as a near-black tile with small "No snapshot" text. Also ruled out: the image hidden behind the video element, the inline player replacing the image early, `has_thumbnail` not being used, and Cloudflare (it caches the unversioned `/static/analytics_workspace.js` for 4 h -- `cf-cache-status: HIT` -- but pages request `?v=<build>` since `330f7be`, and those URLs served the new code).

**Real media coverage found (14 days)**: Smart Motion 79,650 events, ~48% with clip + snapshot; **PPE 10,156, Facial Recognition 360, People Counting 178 -- zero with any stored media**; LPR, Line Crossing, Intrusion -- no events on staging.

**Shipped**: golden `2ccbe34` = `eb3002e` + `b5611cc` only (7 files; tree identical to `b5611cc`): `/api/customer/events/{cam}/{id}/thumbnail?size=card` serves the same stored snapshot scaled to 480 px in memory (nothing new stored, bounded per-process cache, `private` browser cache, falls back to the normal redirect); cards show "Loading preview..." -> snapshot, or "No preview available" (never a black tile); first screenful eager, rest lazy; the snapshot is the inline player's poster until the clip plays.

**Deploy**: `aws ssm send-command` to `i-0a082abd812929bb4`. `/opt/anyaicam-staging-build/anyaicam-eb3002e` (verified byte-identical to `eb3002e` for all touched files) copied to `anyaicam-2ccbe34`, the `eb3002e..2ccbe34` patch applied, all 7 files `sha256sum -c` OK against the git blobs; `docker build --label anyaicam.commit=2ccbe340dccb...` -> `deploy-portal:2ccbe34` (`sha256:464cb15a...`). Cutover with the same network/aliases (`deploy_default`, `portal`/`vms`), env-file, mounts, command and restart policy, `-e ANYAICAM_BUILD_ID=2ccbe340dccb6c40451a48e4f5acf611eada8a1f`. The first attempt auto-rolled back on a false negative: the in-container health probe hit `127.0.0.1`, which the trusted-hosts check rejects (400) while real traffic was being served normally; left stopped as `portal-2ccbe34-failed`. Re-run with `Host: portal-staging.anyaicam.com`: healthy in ~6 s, `RestartCount=0`. Rollback kept (stopped, not deleted): `portal-eb3002e-pre-2ccbe34-20260926T031338Z-rollback`. Two brief (seconds) interruptions total.

**Verified on staging (signed-in customer session, real data)**: `/health` build `2ccbe340dccb...`; page loads `analytics_workspace.js`/`inline_media.css` `?v=2ccbe340dccb`. Smart Motion, desktop 1366 px: 48 cards; every preview request `?size=card`, 480x270, 14-20 KB (was 150-860 KB), first screenful decoded within ~1 s; no-media results show "No preview available" on a slate tile; card text carries camera, local time, analytic label and confidence. Click -> clip plays inline under the card (1280 px source) with the card snapshot as poster; switching to another result keeps a single player; controls intact (Pause, Mute, Fullscreen, Download, Share link, Open in Playback, Close player). Opening a no-media result shows "No preview available" / "No snapshot or clip was recorded for this detection." PPE page: all cards "No preview available" (correct -- no media exists). Phone 390 px: snapshot visible before tap, no horizontal scroll, tap plays inline. Only console error: Cloudflare Insights beacon blocked by CSP (pre-existing, unrelated). Local: `test_analytics_workspace_browser.py` 16/16 (Chromium + Edge, desktop + phone, `ANYAICAM_RUN_BROWSER_TESTS=1`) and `test_event_thumbnail_card_size.py` pass.

**Pushed**: `reconcile/golden-foundation-20260911` @ `2ccbe34`, `ui/unified-camera-tiles-20260925` @ `b5611cc`.
