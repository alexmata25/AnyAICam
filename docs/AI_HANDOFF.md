...
**What RDM-2 Groups 2C–2G deliberately did NOT do:** no live deployment, no real service restart, no migration run against a live database, and no AWS resource creation/modification or IAM changes. `app/appliance_protocol.py` was not modified. Group 2H (integration/E2E tests) has not been started.
