...
- `get_configured_signer()` intentionally, unconditionally returns `None` in this phase — no Secrets Manager retrieval exists yet, so every real production call path fails closed by construction until a future phase creates actual CloudFront key material. (This described Phase 6b's original state only; superseded 2026-08-17 — see the "CloudFront signing infrastructure (AWS) and application-side signer integration" entry further below, where `get_configured_signer()` was activated. This bullet is left as originally written to preserve an accurate record of Phase 6b as committed.)
...
### Phase 6e result — idle-relay auto-stop, concurrency/idempotency hardening, observability, IMPLEMENTED, TESTED, AND COMMITTED (NOT YET PUSHED as of this entry; subsequently pushed — see below) (2026-08-16)

Phase 6e was authorized after an investigation found that the original 8-section Phase 6e proposal's reconnect/transient-failure, segment-drop, credential-renewal-failure, and CDN/HLS-gap sections (1, 3, 4) conflicted with this section's own already-approved "Reconnect/failure behavior" design record. Scope was narrowed, on that finding, to: idle-relay auto-stop (already part of the approved-but-previously-unbuilt "Continuous vs. on-demand publishing" item 5 above), concurrency/idempotency hardening around it, failure-state observability for it, and regression testing — explicitly preserving the existing approved reconnect, segment-drop, credential-renewal, and HLS/CDN behavior unchanged.

**Files changed:**
- `app/live_relay_idle_sweep.py` (new) — idle detection, tracking, and the atomic claim-and-stop worker.
- `app/tests/test_live_relay_idle_sweep.py` (new) — 12 focused tests.
- `app/live_view_sessions.py` — one additive line in `start_live_view()`.
- `app/main.py` — one import + `lifespan()` wiring, mirroring `live_relay_uploader.py`'s own task exactly.
- `app/db_migrations.py` — one new migration, `live_relay_idle_tracking`.
- `app/tests/test_live_view_sessions.py` — fixture extended with the new table, plus 2 new regression tests.

**Idle-relay auto-stop design:**
- A camera is "idle" once it has zero `live_view_sessions` rows in state `requested` — derived entirely from state `live_view_sessions.py` already maintains; no new heartbeat/polling signal, and `app/live_playlist.py` is untouched.
- New table `live_relay_idle_tracking(camera_id PK, appliance_id, idle_since, stop_queued_at)`: one row per currently-idle-or-already-handled camera.
- Phase A (`_discover_idle_candidates`): finds cameras with a session history but currently zero active sessions.
- Phase B (`_track_idle_camera`): idempotently starts tracking (`INSERT OR IGNORE`) — `idle_since` is set once, never reset by repeated observation.
- Phase C (`_claim_and_stop_if_still_idle`): the sole correctness boundary. An atomic conditional `UPDATE live_relay_idle_tracking SET stop_queued_at=? WHERE ... AND stop_queued_at IS NULL` claims the right to queue this idle cycle's one stop; only the transaction that flips `NULL` to non-`NULL` wins. The claiming transaction then re-checks `live_view_sessions` for active sessions in the same transaction — if a viewer is active (a stale Phase A/B snapshot), the tracking row is deleted and no stop is queued; otherwise `stop_live_relay` is queued exactly once and the tracking row is retained (not deleted), permanently excluding it from every later claim attempt.
- `IDLE_GRACE_PERIOD_SECONDS = 30`, `IDLE_SWEEP_INTERVAL_SECONDS = 10` — fixed constants, not env-overridable, matching what was explicitly approved.
- Worker `live_relay_idle_sweep_worker()`, wired into `app/main.py`'s `lifespan()`, gated `RUNTIME_ROLE in {cloud, combined}` — the cloud-side counterpart to `live_relay_uploader.py`'s own `RUNTIME_ROLE in {edge, combined}`-gated worker, same tick-and-sleep shape, same defensive internal re-check.
- `start_live_view()` deletes any existing tracking row for the camera in the same transaction as the new session insert and `start_live_relay` command — a returning viewer always establishes a fresh idle-tracking lifecycle, cancelling any in-progress grace-period clock.

**Concurrency/idempotency hardening:**
- At most one automatic `stop_live_relay` per idle cycle, guaranteed by the atomic claim (not application-level locking) — proven by a test that runs the sweep an unbounded number of additional times after the first stop and asserts the command count never grows.
- Phase C's re-check, inside the same transaction as its claim, prevents a stale stop against any viewer session that has already committed by the moment the re-check runs — this is what actually cleans up a stale Phase A/B snapshot. It does not eliminate every timing case: if the claim transaction commits an instant before a concurrent `start_live_view()` transaction begins, a stop can still be queued right as a new viewer starts. This narrow boundary case is self-correcting, not a permanently incorrect outcome — the resulting `start_live_relay` command is applied by the appliance's existing, unchanged command-reconciliation logic after the stop, and the relay ends up active.
- An idle-sweep-triggered stop racing with a customer's own explicit stop is harmless by construction — both are independent, idempotent `stop_live_relay` commands (tagged via `appliance_commands.created_by`: `live-relay-idle-sweep` vs. `customer-live-view`); the appliance's existing, unchanged command-reconciliation logic already tolerates redundant or out-of-order stop/start commands.
- Repeated `start_live_view()` calls retain their existing (pre-Phase-6e) behavior unchanged — not deduplicated, an explicit scope decision to avoid a `live_view_sessions.py` behavior change beyond hardening.

**Observability:**
- `appliance_commands.created_by='live-relay-idle-sweep'` distinguishes idle-sweep-originated stop commands from customer-originated ones in existing history/audit queries, with no schema change beyond the new table.
- A structured log line (`live_relay.idle_stop_queued camera_id=... idle_seconds=...`) and a `partner_db.audit()` entry (`system.live_view_idle_stopped`, synthetic `{'email': 'system', 'role': 'live-relay-idle-sweep'}` actor) are recorded on every automatic stop.
- Confirmed by test: the queued command's payload never contains any of `appliance_protocol.py`'s `FORBIDDEN_KEYS`.

**Preserved, unchanged behavior (explicit non-goals, verified by regression):** `app/live_relay_uploader.py`'s segment-drop-not-retry and silent credential-renewal-stall behavior; `app/live_playlist.py` and `app/live_cdn_signing.py`; `app/live_view_page.py`'s client-side polling/reconnect/retry logic. None of these files were touched by Phase 6e.

**Testing:**
- Focused tests (`test_live_relay_idle_sweep.py` + `test_live_view_sessions.py`): **33/33 passing**, on the real tree.
- Full `app/tests/` suite, verified in an isolated mirror of pushed HEAD (`44efb7e`) and again against the real repo after application: **312 tests total, 308 passing, 0 failures, 4 errors** — the same 4 pre-existing, unrelated environment-only `PermissionError: [Errno 13] Permission denied: '/app'` errors present at baseline (298 total / 294 passing / 4 errors). Zero Phase 6e regressions; 312 = 298 baseline + 12 new idle-sweep tests + 2 new `live_view_sessions` tests.
- `py_compile` clean on all six touched/new files, aside from the same pre-existing, unrelated `SyntaxWarning` already present in `main.py` before this phase.

**What Phase 6e deliberately does NOT do:** no reconnect/transient-failure handling, segment-drop retry, credential-renewal failure-state, or CDN/HLS gap-recovery logic of any kind — all four remain exactly as this section's already-approved "Reconnect/failure behavior" design record describes; no AWS/IAM changes; no changes to `app/live_relay_uploader.py`, `app/live_playlist.py`, `app/live_cdn_signing.py`, or `app/live_view_page.py`; no `IDLE_GRACE_PERIOD_SECONDS`/`IDLE_SWEEP_INTERVAL_SECONDS` env-var overrides; no deduplication of repeated `start_live_view()` calls; no fix to the `/customer-account` → `/customer-portal` auth-system mismatch or the `RUNTIME_ROLE=edge`-vs-`ecs-task-definition.json` production drift (both remain separately flagged from Phase 6d).

**Committed as `bc39bb0`** ("Phase 6e: idle-relay auto-stop, concurrency hardening, observability"). **Not yet pushed as of this entry** — subsequently pushed to `origin/build/v1.2-modular-foundation`; verified via `git merge-base --is-ancestor` an ancestor of `origin/build/v1.2-modular-foundation` at `24125bd` (2026-08-17).

### Phase 6f result — per-appliance live-relay pilot gating, IMPLEMENTED, TESTED, AND COMMITTED (NOT YET PUSHED as of this entry; subsequently pushed — see below) (2026-08-16)

Phase 6f was authorized after an investigation found no explicit scope defined for it anywhere in this document — only the recurring "(6a–6f)" labeling and the closing authorization boundary. Scope was inferred from original item 8 of this section's own pre-lettering "Implementation phases" list ("Gradual production rollout behind the feature flag, one pilot appliance at a time, before flipping the default"), narrowed to the one concrete, AWS-free, code-only piece of that item that was genuinely missing: `ANYAICAM_LIVE_RELAY_ENABLED` is a single global, process-level flag with no per-appliance granularity, so "one pilot appliance at a time" was not achievable before this phase. A separate, unrelated legacy scaffold bundle (`README_PHASE6F.md`, `app/apply_phase6f.py`, `phase6f_extension.py.txt`, `app/test_phase6f_static.py`, `phase6f_verification_report.md`, present since the initial commit `1eee376`, 2026-08-05, referencing a different fictional "Phase 6E/6F" marker string never present in this project's real `main.py`) was investigated and confirmed disconnected from this numbering — not used, not referenced, not modified.

**Files changed:**
- `app/db_migrations.py` — one additive, fail-closed column: `appliances.live_relay_pilot INTEGER NOT NULL DEFAULT 0`.
- `app/appliance_cloud.py` — `live_relay_session()`'s gate extended; new administrator-only route.
- `app/tests/test_live_relay_session_endpoints.py` — 10 new pilot-gating/admin-route tests.
- `app/tests/test_live_relay_resource_scaling.py` (new) — 6 new code-level characterization tests.

**Gate semantics — the single authoritative capability boundary:**
- `live_relay_session()` (`POST /api/appliance/live/{camera_id}/session`, the only route that ever mints a real AWS STS credential) now requires **both** the global `ANYAICAM_LIVE_RELAY_ENABLED` master kill switch **and** `appliance.live_relay_pilot=1` — neither alone is sufficient. The same 404 status and message (`"Live relay is not enabled."`) is reused for both "globally off" and "not in the pilot," so a rejected appliance can never distinguish the two.
- No duplicate gate was added to `live_relay_segment_available()`, `live_view_sessions.start_live_view()`/`stop_live_view()`, or `live_playlist.py` — all bookkeeping-only, already-scoped, or already covered by existing timeout/retry UX; a second check there would duplicate policy without closing a real gap. Zero appliance-side code changes were needed: `live_relay_uploader.py`'s `_control_plane_request()` already absorbs a pilot-ineligible 404 as an ordinary, status-code-agnostic credential-fetch failure, identical to the existing, approved "credential-renewal failure" design record.
- New route `POST /api/admin/appliances/{appliance_id}/live-relay-pilot`, `administrator`-only via `require_partner_access()`, mirrors `admin_activation_token`/`revoke_appliance`'s existing shape with one deliberate divergence: an unknown `appliance_id` returns `404` and never reaches `audit()` — detected via the `UPDATE`'s own `rowcount` against the `appliances.id` primary key (the same atomic-rowcount idiom Phase 6e's `live_relay_idle_sweep.py` already established), not `revoke_appliance()`'s existing silent-no-op shape.

**Testing:**
- Focused tests (`test_live_relay_session_endpoints.py` + `test_live_relay_resource_scaling.py`): **27/27 passing**, on the real tree.
- Full `app/tests/` suite, verified in an isolated mirror of pushed HEAD (`55241fb`) and again against the real repo after application: **328 tests total, 324 passing, 0 failures, 4 errors** — the same 4 pre-existing, unrelated environment-only `PermissionError: [Errno 13] Permission denied: '/app'` errors present at baseline (312 total / 308 passing / 4 errors). Zero Phase 6f regressions; 328 = 312 baseline + 10 pilot-gating/admin-route tests + 6 characterization tests.
- `py_compile` clean on all four touched/new files.

**Characterization findings — code-level, not a production load test:** `test_live_relay_resource_scaling.py`'s docstring and assertions are explicit that these are code-level characterization tests — direct function calls against a mocked boto3 boundary in one process, no real network/ASGI server, no real AWS, no real ECS.
- **Proven:** no live-relay control-plane route (`appliance_cloud.py`, `live_playlist.py`, `live_cdn_signing.py`) contains a `StreamingResponse`, `FileResponse`, media-file read, or S3 `get_object` call anywhere. Across 50 simulated concurrent cameras, the only boto3 client ever constructed is `"sts"` (credential issuance) — never `"s3"` or any data-plane call — and every `segment-available` response stays bounded to `{status, segment_count}` regardless of N. This demonstrates, at the code level, that FastAPI/ECS does not proxy live media bytes and that concurrent viewers do not create per-viewer media transfer through the application server.
- **Honest, un-fixed finding, recorded as a pre-existing scaling limitation/follow-up, not addressed by Phase 6f:** `live_manifest.py`'s `record_segment()` (Phase 2, unchanged) serializes every camera's segment-available call through one process-wide `Lock()` and rewrites one shared JSON file containing every tracked camera's state on each call. Measured on the real tree: **~0.041ms at N=1 tracked camera vs. ~0.066ms at N=50 tracked cameras** — real, measurable growth, not media-byte proxying, but real application-server work whose cost is shaped by total concurrent camera count. This is a candidate for a later, separately-authorized follow-up; fixing it would mean redesigning an already-approved Phase 2 module, out of Phase 6f's scope.

**What Phase 6f deliberately does NOT do:** no AWS/IAM changes of any kind; no CloudFront enablement, signing keys, public keys, key groups, or Secrets Manager resource creation; no change to any real environment or production feature-flag value; no actual pilot activation (the admin toggle mechanism was built and tested against an in-memory fixture only, never invoked against any real appliance); no changes to `app/live_relay_uploader.py`, `app/live_playlist.py`, `app/live_cdn_signing.py`, `app/live_view_sessions.py`, `app/live_view_page.py`, or `app/live_relay_idle_sweep.py`; no changes to the legacy `README_PHASE6F.md`/`app/apply_phase6f.py` scaffold bundle (confirmed disconnected, left untouched); no fix to the `/customer-account` → `/customer-portal` auth-system mismatch or the `RUNTIME_ROLE=edge`-vs-`ecs-task-definition.json` production drift (both remain separately flagged).

**Committed as `1d5722a`** ("Phase 6f: per-appliance live-relay pilot gating"). **Not yet pushed as of this entry** — subsequently pushed to `origin/build/v1.2-modular-foundation`; verified via `git merge-base --is-ancestor` an ancestor of `origin/build/v1.2-modular-foundation` at `24125bd` (2026-08-17).

### CloudFront signing/enablement follow-up investigation — HOLD (2026-08-16)

Authorized as investigation-only, after Phase 6f, to determine what enabling CloudFront distribution `E1LBM1M5362SXK` for real signed playback would require. Grounded throughout at pushed/synchronized HEAD `fe000b9`; the 19 pre-existing unrelated working-tree modifications remained untouched.

**Documented-state review (no live verification):** identified the Phase 5 documented state of `E1LBM1M5362SXK` (`Enabled=false`, OAC-scoped origin, no trusted key group/public key/signing infrastructure) and Phase 6b's `app/live_cdn_signing.py`/`app/live_playlist.py` as the exact code seam (`get_configured_signer()` unconditionally `None`; `ANYAICAM_CLOUDFRONT_URL`/`ANYAICAM_CLOUDFRONT_KEY_PAIR_ID` already read but no env var yet exists for private-key retrieval). This is a restatement of what was already documented under Phase 5/6b above — **no current AWS state was verified as part of this investigation.**

**Live read-only verification — attempted only far enough to establish AWS access, then stopped:** the AWS CLI is not installed in this environment, no AWS credential/config environment (env vars or `~/.aws/`) is present, and `boto3`'s `sts.get_caller_identity()` failed with `NoCredentialsError`. **No CloudFront, S3, IAM, public-key/key-group, or Secrets Manager resource was inspected live as a result** — none could be. The documented 2026-08-14 AWS state above must not be represented as currently verified. No AWS mutation occurred, no secret value was requested or retrieved, and no key material was generated at any point.

**Design findings only, not authorized implementation:** the investigation identified the likely future signing requirements — a CloudFront public key, key group, and trusted-key-group association; private signing-key storage/retrieval (Secrets Manager, no env var defined yet); least-privilege IAM (a dedicated `secretsmanager:GetSecretValue`-only role, resource-scoped to one secret ARN, assumed by the existing EC2 app role rather than granted directly, mirroring the already-approved Phase 1 two-role pattern); and activating `get_configured_signer()` in application code. **Enabling the distribution alone is not sufficient** — trusted-key-group enforcement and the relevant distribution/cache-behavior wiring must be verified/designed and in place before enablement, or an enabled distribution would serve every request unsigned.

**Proposed rollout remains fail-closed:** verify live AWS state first (read-only) → prepare signing infrastructure (key pair, key group, secret, IAM role) while the distribution remains disabled → implement and test signer-loading application code separately, still with the distribution disabled → only then consider enabling CloudFront and exercising exactly one Phase 6f pilot appliance, each step under its own separate, explicit authorization.

**Current state at the time of this entry: HOLD.** No further AWS investigation, CloudFront work, implementation, repository changes, or deployment work was authorized until an explicit new go-ahead — see the follow-up entry immediately below for what has since been separately authorized and completed.

### CloudFront signing infrastructure (AWS) and application-side signer integration — COMPLETE AND PUSHED (2026-08-17)

**AWS signing infrastructure provisioned under separate, explicit authorization; distribution still disabled:** since the HOLD entry above, the S3 `live/` prefix lifecycle (1-day expiration + 1-day incomplete-multipart-upload cleanup) was restored; a CloudFront public key (`K3P6E9C1Q4X8G7`) and key group (`1b9e753d-917a-42b0-b8c4-81f149d4aafb`) were created, with the trusted key group attached to distribution `E1LBM1M5362SXK`'s default behavior and deployed; a Secrets Manager secret (`anyaicam/live-relay/cloudfront-private-key`, region `us-east-1`) holds the CloudFront private key; and a dedicated signing-key-reader IAM role (`arn:aws:iam::880690594006:role/anyaicam-cloudfront-signing-key-reader`), scoped to `secretsmanager:GetSecretValue` on only that one secret, was created and made assumable by the existing `anyaicam-ec2-app-role` — mirroring the same least-privilege two-role pattern already approved for Phase 1's live-upload credential. **Distribution `E1LBM1M5362SXK` remains `Enabled=false`** — none of this AWS work made the distribution live or serves any object externally.

**Application-side signer integration IMPLEMENTED, TESTED, COMMITTED, AND PUSHED (2026-08-17):** `app/live_cdn_signing.py`'s `get_configured_signer()` is now active instead of the Phase 6b unconditional fail-closed stub — it assumes the signing-key-reader role above via STS, uses that role's temporary credentials to call Secrets Manager `GetSecretValue` on the private-key secret in `us-east-1`, and wraps the returned PEM into the existing `cryptography_rsa_signer()` callback. The constructed signer is cached at module level with thread-safe, lazy, process-lifetime caching (a successful fetch is reused for every later call in the process; a failed fetch is never permanently cached, so the next call retries from scratch). `sign_segment_url()`'s behavior is unchanged: the fixed 20-second signed-segment URL TTL, the canonical-filename validation, the per-component percent-encoding, and the `live_relay_s3_prefix()` drift guard are all untouched. Every failure mode — missing configuration, STS failure, Secrets Manager failure, missing/empty/malformed PEM, or signer-construction failure — still fails closed by returning `None`, exactly as `live_playlist.py`'s existing `503` handling already expects. Private-key PEM material, temporary AWS credentials, and session tokens are never included in any log message or exception. `app/tests/test_live_cdn_signing.py` was expanded with focused coverage for STS assume-role, Secrets Manager retrieval, missing/malformed secret content, fail-closed behavior, caching, retry-after-failure, real RSA signature verification, and a negative-content check proving private-key/credential values never appear in log output.

**Verification:** 59/59 passing across the three directly relevant modules (`test_live_cdn_signing.py`, `test_live_playlist.py`, `test_live_relay_resource_scaling.py`); the full `app/tests/` suite: 338 total / 334 passing / 4 errors — the same 4 pre-existing, unrelated environment-only `PermissionError: [Errno 13] Permission denied: '/app'` characterization errors already documented under Phase 6e/6f, zero new failures or errors; a `git diff --check` scoped to the two changed files is clean. **Commit contains exactly** `app/live_cdn_signing.py` and `app/tests/test_live_cdn_signing.py` — committed as `24125bd` ("Phase 6b follow-up: activate get_configured_signer() with STS+Secrets Manager") on `build/v1.2-modular-foundation`, and `origin/build/v1.2-modular-foundation` is confirmed synchronized at `24125bd`.

**What this does NOT do:** CloudFront distribution `E1LBM1M5362SXK` has not been enabled and remains `Enabled=false`; no CloudFront enablement, cache-behavior change, or other AWS mutation occurred as part of the application-side commit; real signed playback is not live — no pilot appliance has been exercised end-to-end against a live, enabled distribution. Enabling the distribution, and any live playback verification, remain separately, explicitly un-authorized.

**Current state: the application-side CloudFront signer integration and its supporting AWS signing infrastructure are complete and pushed; CloudFront enablement itself, and any real playback, remain NOT authorized until a further, separate, explicit go-ahead.**
