...
**What RDM-2 Groups 2C/2D/2E deliberately did NOT do:** no live deployment, no real service restart, no migration run against a live database, and no AWS changes. `app/appliance_protocol.py` was not modified. Groups 2F (real health check), 2G (real update source/IAM/S3), and 2H (integration/E2E tests) have not been started.
