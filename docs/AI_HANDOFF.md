...
## 11. Remote Device Management (RDM-2) — device-side integration + cloud-side reporting (Groups 2A–2D IMPLEMENTED, TESTED, COMMITTED, AND PUSHED — 2026-08-15)

RDM-2 continues RDM-1 (§10): wiring the already-built `UpdateStateMachine` into the real appliance-agent startup/command path, then into the cloud's own appliance API, across an approved 2A–2H group sequence. Each group was built scratch-first, verified in an isolated mirror, and reviewed as a complete diff before being written to the real repository — the same discipline used throughout RDM-1. **Groups 2A–2D are now complete, committed, and pushed. Groups 2E and later have not been started.**
...
**What RDM-2 Groups 2C/2D deliberately did NOT do:** no live deployment, no real service restart, no migration run against a live database, and no AWS changes. `app/appliance_protocol.py` was not modified. Groups 2E (appliance-side post-restart reporting), 2F (real health check), 2G (real update source/IAM/S3), and 2H (integration/E2E tests) have not been started.
